...
public class KeccakDigest implements ExtendedDigest {
    ...
    private void KeccakAbsorb(byte[] bArr, int r9) {
        int r0 = this.rate >>> 6;
    ...
    private void KeccakPermutation() {
        ...
        while (r2 < r14) {
            ...
            j21 = j67 ^ ((~j76) & j65);
            long j103 = j85 ^ KeccakRoundConstants[r2];
            r2++;
            j7 = j89;
            j13 = j94;
            j12 = j93;
    ...
    private void init(int r2) {
        if (r2 != 128 && r2 != 224 && r2 != 256 && r2 != 288 && r2 != 384 && r2 != 512) {
            throw new IllegalArgumentException("bitLength must be one of 128, 224, 256, 288, 384, or 512.");
        }
        initSponge(1600 - (r2 << 1));
    }

    private void initSponge(int r7) {
        if (r7 <= 0 || r7 >= 1600 || r7 % 64 != 0) {
        ...
        while (true) {
            ...
            if (r2 >= jArr.length) {
                ...
                this.fixedOutputLength = (1600 - r7) / 2;
                return;
            }
            jArr[r2] = 0;
            r2++;
    ...
    private void padAndSwitchToSqueezingPhase() {
        ...
        int r1 = this.bitsInQueue;
        int r2 = r1 >>> 3;
        bArr[r2] = (byte) (bArr[r2] | ((byte) (1 << (r1 & 7))));
        int r12 = r1 + 1;
        this.bitsInQueue = r12;
        if (r12 == this.rate) {
            KeccakAbsorb(bArr, 0);
        } else {
            int r0 = r12 >>> 6;
            int r13 = r12 & 63;
            int r4 = 0;
            for (int r22 = 0; r22 < r0; r22++) {
                long[] jArr = this.state;
                jArr[r22] = jArr[r22] ^ Pack.littleEndianToLong(this.dataQueue, r4);
                r4 += 8;
            }
            if (r13 > 0) {
                long[] jArr2 = this.state;
                jArr2[r0] = (((1 << r13) - 1) & Pack.littleEndianToLong(this.dataQueue, r4)) ^ jArr2[r0];
            }
        }
        long[] jArr3 = this.state;
        int r14 = (this.rate - 1) >>> 6;
        jArr3[r14] = jArr3[r14] ^ Long.MIN_VALUE;
        this.bitsInQueue = 0;
        this.squeezing = true;
    }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void absorb(byte[] bArr, int r7, int r8) {
        int r2;
        int r0;
        int r6;
        ...
        int r22 = r1 - r03;
        if (r8 < r22) {
            System.arraycopy(bArr, r7, this.dataQueue, r03, r8);
            r6 = this.bitsInQueue + (r8 << 3);
        } else {
            if (r03 > 0) {
                System.arraycopy(bArr, r7, this.dataQueue, r03, r22);
                r2 = r22 + 0;
            ...
            while (true) {
                ...
                KeccakAbsorb(bArr, r7 + r2);
                r2 += r1;
            }
            System.arraycopy(bArr, r7 + r2, this.dataQueue, 0, r0);
            r6 = r0 << 3;
        }
        this.bitsInQueue = r6;
    }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void absorbBits(int r5, int r6) {
        if (r6 < 1 || r6 > 7) {
        ...
        this.dataQueue[r1 >>> 3] = (byte) (r5 & ((1 << r6) - 1));
        this.bitsInQueue = r1 + r6;
    }

    @Override // org.bouncycastle.crypto.Digest
    public int doFinal(byte[] bArr, int r4) {
        squeeze(bArr, r4, this.fixedOutputLength);
        reset();
        return getDigestSize();
    }

    /* JADX INFO: Access modifiers changed from: protected */
    public int doFinal(byte[] bArr, int r2, byte b, int r4) {
        if (r4 > 0) {
            absorbBits(b, r4);
        }
        squeeze(bArr, r2, this.fixedOutputLength);
        reset();
        return getDigestSize();
    }
    ...
    /* JADX INFO: Access modifiers changed from: protected */
    public void squeeze(byte[] bArr, int r11, long j) {
        if (!this.squeezing) {
        ...
        while (j2 < j) {
            ...
            System.arraycopy(this.dataQueue, (this.rate - this.bitsInQueue) / 8, bArr, ((int) (j2 / 8)) + r11, min / 8);
