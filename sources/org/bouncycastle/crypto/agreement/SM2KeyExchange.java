...
public class SM2KeyExchange {
    ...
    /* renamed from: S1 */
    private byte[] m127S1(Digest digest, ECPoint eCPoint, byte[] bArr) {
    ...
    /* renamed from: S2 */
    private byte[] m126S2(Digest digest, ECPoint eCPoint, byte[] bArr) {
    ...
    private byte[] kdf(ECPoint eCPoint, byte[] bArr, byte[] bArr2, int r15) {
        ...
        while (r5 < r152) {
            ...
            r8++;
            Pack.intToBigEndian(r8, bArr3, 0);
            this.digest.update(bArr3, 0, 4);
            this.digest.doFinal(bArr3, 0);
            int min = Math.min(digestSize, r152 - r5);
            System.arraycopy(bArr3, 0, bArr4, r5, min);
            r5 += min;
        }
        return bArr4;
    }
    ...
    private BigInteger reduce(BigInteger bigInteger) {
        return bigInteger.and(BigInteger.valueOf(1L).shiftLeft(this.f1683w).subtract(BigInteger.valueOf(1L))).setBit(this.f1683w);
    }

    public byte[] calculateKey(int r5, CipherParameters cipherParameters) {
        ...
        return this.initiator ? kdf(calculateU, z, z2, r5) : kdf(calculateU, z2, z, r5);
    }
    ...
    public byte[][] calculateKeyWithConfirmation(int r16, byte[] bArr, CipherParameters cipherParameters) {
        ...
        ECPoint calculateU = calculateU(sM2KeyExchangePublicParameters);
        if (!this.initiator) {
            byte[] kdf = kdf(calculateU, z2, z, r16);
            ...
            return new byte[][]{kdf, m127S1(this.digest, calculateU, calculateInnerHash), m126S2(this.digest, calculateU, calculateInnerHash)};
        }
        byte[] kdf2 = kdf(calculateU, z, z2, r16);
        ...
        if (Arrays.constantTimeAreEqual(m127S1(this.digest, calculateU, calculateInnerHash2), bArr)) {
            return new byte[][]{kdf2, m126S2(this.digest, calculateU, calculateInnerHash2)};
        }
    ...
    public void init(CipherParameters cipherParameters) {
        ...
        this.f1683w = (this.ecParams.getCurve().getFieldSize() / 2) - 1;
    }
}
