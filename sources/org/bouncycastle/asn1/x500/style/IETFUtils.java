...
public class IETFUtils {
    public static void appendRDN(StringBuffer stringBuffer, RDN rdn, Hashtable hashtable) {
        ...
        boolean z = true;
        for (int r2 = 0; r2 != typesAndValues.length; r2++) {
            if (z) {
                z = false;
            } else {
                stringBuffer.append('+');
            }
            appendTypeAndValue(stringBuffer, typesAndValues[r2], hashtable);
        }
    }
    ...
    */
    ...
    public static boolean rDNAreEqual(RDN rdn, RDN rdn2) {
        ...
        for (int r0 = 0; r0 != typesAndValues.length; r0++) {
            if (!atvAreEqual(typesAndValues[r0], typesAndValues2[r0])) {
                return false;
            }
        }
        return true;
    ...
    private static ASN1ObjectIdentifier[] toOIDArray(Vector vector) {
        ...
        for (int r2 = 0; r2 != size; r2++) {
            aSN1ObjectIdentifierArr[r2] = (ASN1ObjectIdentifier) vector.elementAt(r2);
        }
    ...
    public static ASN1Encodable valueFromHexString(String str, int r6) throws IOException {
        ...
        for (int r2 = 0; r2 != length; r2++) {
            ...
            bArr[r2] = (byte) (convertHex(str.charAt(r3 + 1)) | (convertHex(charAt) << 4));
        }
    ...
    public static String valueToString(ASN1Encodable aSN1Encodable) {
        ...
        int r6 = 2;
        r6 = (stringBuffer.length() >= 2 && stringBuffer.charAt(0) == '\\' && stringBuffer.charAt(1) == '#') ? 0 : 0;
        while (r6 != length) {
            char charAt = stringBuffer.charAt(r6);
        ...
        if (stringBuffer.length() > 0) {
            for (int r4 = 0; stringBuffer.length() > r4 && stringBuffer.charAt(r4) == ' '; r4 += 2) {
                stringBuffer.insert(r4, "\\");
            }
        }
