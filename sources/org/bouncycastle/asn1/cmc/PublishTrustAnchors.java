...
public class PublishTrustAnchors extends ASN1Object {
    ...
    public PublishTrustAnchors(BigInteger bigInteger, AlgorithmIdentifier algorithmIdentifier, byte[][] bArr) {
        ...
        for (int r4 = 0; r4 != bArr.length; r4++) {
            aSN1EncodableVector.add(new DEROctetString(Arrays.clone(bArr[r4])));
        }
    ...
    public byte[][] getAnchorHashes() {
        ...
        for (int r2 = 0; r2 != size; r2++) {
            bArr[r2] = Arrays.clone(ASN1OctetString.getInstance(this.anchorHashes.getObjectAt(r2)).getOctets());
        }
        return bArr;
    }
