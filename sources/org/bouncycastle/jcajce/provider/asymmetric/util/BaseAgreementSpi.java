...
public abstract class BaseAgreementSpi extends KeyAgreementSpi {
    ...
    private byte[] getSharedSecretBytes(byte[] bArr, String str, int r8) throws NoSuchAlgorithmException {
        ...
        if (derivationFunction == null) {
            ...
            if (!(derivationFunction instanceof DHKEKGenerator)) {
                ...
                try {
                    kDFParameters = new DHKDFParameters(new ASN1ObjectIdentifier(str), r8, bArr, this.ukmParameters);
                } catch (IllegalArgumentException unused) {
                    throw new NoSuchAlgorithmException("no OID for algorithm: " + str);
                }
            }
            this.kdf.init(kDFParameters);
            this.kdf.generateBytes(bArr3, 0, r2);
            Arrays.clear(bArr);
            return bArr3;
        }
    }
    ...
    public static byte[] trimZeroes(byte[] bArr) {
        ...
        int r1 = 0;
        while (r1 < bArr.length && bArr[r1] == 0) {
            r1++;
        }
        int length = bArr.length - r1;
        byte[] bArr2 = new byte[length];
        System.arraycopy(bArr, r1, bArr2, 0, length);
        return bArr2;
    }
    ...
    public int engineGenerateSecret(byte[] bArr, int r5) throws IllegalStateException, ShortBufferException {
        byte[] engineGenerateSecret = engineGenerateSecret();
        if (bArr.length - r5 >= engineGenerateSecret.length) {
            System.arraycopy(engineGenerateSecret, 0, bArr, r5, engineGenerateSecret.length);
