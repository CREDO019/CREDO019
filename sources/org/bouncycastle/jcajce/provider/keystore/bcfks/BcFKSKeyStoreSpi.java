...
import org.bouncycastle.asn1.p029bc.EncryptedObjectStoreData;
import org.bouncycastle.asn1.p029bc.EncryptedPrivateKeyData;
import org.bouncycastle.asn1.p029bc.EncryptedSecretKeyData;
import org.bouncycastle.asn1.p029bc.ObjectData;
import org.bouncycastle.asn1.p029bc.ObjectDataSequence;
import org.bouncycastle.asn1.p029bc.ObjectStore;
import org.bouncycastle.asn1.p029bc.ObjectStoreData;
import org.bouncycastle.asn1.p029bc.ObjectStoreIntegrityCheck;
import org.bouncycastle.asn1.p029bc.PbkdMacIntegrityCheck;
import org.bouncycastle.asn1.p029bc.SecretKeyData;
import org.bouncycastle.asn1.p029bc.SignatureCheck;
...
public class BcFKSKeyStoreSpi extends KeyStoreSpi {
    ...
    private EncryptedPrivateKeyData createPrivateKeySequence(EncryptedPrivateKeyInfo encryptedPrivateKeyInfo, Certificate[] certificateArr) throws CertificateEncodingException {
        ...
        for (int r1 = 0; r1 != certificateArr.length; r1++) {
            certificateArr2[r1] = org.bouncycastle.asn1.x509.Certificate.getInstance(certificateArr[r1].getEncoded());
        }
    ...
    private byte[] generateKey(KeyDerivationFunc keyDerivationFunc, String str, char[] cArr, int r10) throws IOException {
        ...
        if (MiscObjectIdentifiers.id_scrypt.equals((ASN1Primitive) keyDerivationFunc.getAlgorithm())) {
            ...
            return SCrypt.generate(Arrays.concatenate(PKCS12PasswordToBytes, PKCS12PasswordToBytes2), scryptParams.getSalt(), scryptParams.getCostParameter().intValue(), scryptParams.getBlockSize().intValue(), scryptParams.getBlockSize().intValue(), r10);
    ...
    private KeyDerivationFunc generatePkbdAlgorithmIdentifier(ASN1ObjectIdentifier aSN1ObjectIdentifier, int r9) {
        byte[] bArr = new byte[64];
        getDefaultSecureRandom().nextBytes(bArr);
        if (PKCSObjectIdentifiers.id_PBKDF2.equals((ASN1Primitive) aSN1ObjectIdentifier)) {
            return new KeyDerivationFunc(PKCSObjectIdentifiers.id_PBKDF2, new PBKDF2Params(bArr, 51200, r9, new AlgorithmIdentifier(PKCSObjectIdentifiers.id_hmacWithSHA512, DERNull.INSTANCE)));
        }
        throw new IllegalStateException("unknown derivation algorithm: " + aSN1ObjectIdentifier);
    }

    private KeyDerivationFunc generatePkbdAlgorithmIdentifier(KeyDerivationFunc keyDerivationFunc, int r9) {
        ...
        if (equals) {
            ...
            getDefaultSecureRandom().nextBytes(bArr);
            return new KeyDerivationFunc(MiscObjectIdentifiers.id_scrypt, new ScryptParams(bArr, scryptParams.getCostParameter(), scryptParams.getBlockSize(), scryptParams.getParallelizationParameter(), BigInteger.valueOf(r9)));
        }
        PBKDF2Params pBKDF2Params = PBKDF2Params.getInstance(parameters);
        byte[] bArr2 = new byte[pBKDF2Params.getSalt().length];
        getDefaultSecureRandom().nextBytes(bArr2);
        return new KeyDerivationFunc(PKCSObjectIdentifiers.id_PBKDF2, new PBKDF2Params(bArr2, pBKDF2Params.getIterationCount().intValue(), r9, pBKDF2Params.getPrf()));
    }

    private KeyDerivationFunc generatePkbdAlgorithmIdentifier(PBKDFConfig pBKDFConfig, int r9) {
        if (!MiscObjectIdentifiers.id_scrypt.equals((ASN1Primitive) pBKDFConfig.getAlgorithm())) {
            ...
            getDefaultSecureRandom().nextBytes(bArr);
            return new KeyDerivationFunc(PKCSObjectIdentifiers.id_PBKDF2, new PBKDF2Params(bArr, pBKDF2Config.getIterationCount(), r9, pBKDF2Config.getPRF()));
        }
        ScryptConfig scryptConfig = (ScryptConfig) pBKDFConfig;
        byte[] bArr2 = new byte[scryptConfig.getSaltLength()];
        getDefaultSecureRandom().nextBytes(bArr2);
        return new KeyDerivationFunc(MiscObjectIdentifiers.id_scrypt, new ScryptParams(bArr2, scryptConfig.getCostParameter(), scryptConfig.getBlockSize(), scryptConfig.getParallelizationParameter(), r9));
    }
        ...
        if (objectData != null) {
            if (objectData.getType().equals(PRIVATE_KEY) || objectData.getType().equals(PROTECTED_PRIVATE_KEY)) {
                ...
                for (int r2 = 0; r2 != length; r2++) {
                    x509CertificateArr[r2] = decodeCertificate(certificateChain[r2]);
                }
                ...
                try {
                    ...
                    if (this.validator == null) {
                        ...
                        for (int r7 = 0; r7 != length; r7++) {
                            x509CertificateArr[r7] = (X509Certificate) createCertificateFactory.generateCertificate(new ByteArrayInputStream(certificates[r7].getEncoded()));
                        }
            ...
            try {
                ...
                if (storeCertificates != null) {
                    ...
                    for (int r5 = 0; r5 != length; r5++) {
                        certificateArr[r5] = org.bouncycastle.asn1.x509.Certificate.getInstance(storeCertificates[r5].getEncoded());
                    }
