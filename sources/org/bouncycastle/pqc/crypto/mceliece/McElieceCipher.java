...
public class McElieceCipher implements MessageEncryptor {
    ...
    private GF2Vector computeMessageRepresentative(byte[] bArr) {
        byte[] bArr2 = new byte[this.maxPlainTextSize + ((this.f2452k & 7) != 0 ? 1 : 0)];
        System.arraycopy(bArr, 0, bArr2, 0, bArr.length);
        bArr2[bArr.length] = 1;
        return GF2Vector.OS2VP(this.f2452k, bArr2);
    }

    private void initCipherDecrypt(McEliecePrivateKeyParameters mcEliecePrivateKeyParameters) {
        this.f2453n = mcEliecePrivateKeyParameters.getN();
        int k = mcEliecePrivateKeyParameters.getK();
        this.f2452k = k;
        this.maxPlainTextSize = k >> 3;
        this.cipherTextSize = this.f2453n >> 3;
    }

    private void initCipherEncrypt(McEliecePublicKeyParameters mcEliecePublicKeyParameters) {
        this.f2453n = mcEliecePublicKeyParameters.getN();
        this.f2452k = mcEliecePublicKeyParameters.getK();
        this.f2455t = mcEliecePublicKeyParameters.getT();
        this.cipherTextSize = this.f2453n >> 3;
        this.maxPlainTextSize = this.f2452k >> 3;
    }
    ...
    public void init(boolean z, CipherParameters cipherParameters) {
        ...
        if (!z) {
            ...
            this.f2454sr = CryptoServicesRegistrar.getSecureRandom();
            ...
            this.f2454sr = parametersWithRandom.getRandom();
    ...
    public byte[] messageDecrypt(byte[] bArr) throws InvalidCipherTextException {
        ...
        GF2Vector OS2VP = GF2Vector.OS2VP(this.f2453n, bArr);
        ...
        return computeMessage((GF2Vector) sInv.leftMultiply(((GF2Vector) ((GF2Vector) gF2Vector.add(syndromeDecode)).multiply(p1)).extractRightVector(this.f2452k)));
    }
    ...
    public byte[] messageEncrypt(byte[] bArr) {
        if (this.forEncryption) {
            ...
            return ((GF2Vector) ((McEliecePublicKeyParameters) this.key).getG().leftMultiply(computeMessageRepresentative).add(new GF2Vector(this.f2453n, this.f2455t, this.f2454sr))).getEncoded();
        }
        throw new IllegalStateException("cipher initialised for decryption");
    }
}
