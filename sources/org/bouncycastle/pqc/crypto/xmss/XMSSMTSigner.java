...
public class XMSSMTSigner implements StateAwareMessageSigner {
            ...
            if (xMSSMTPrivateKeyParameters != null) {
                synchronized (xMSSMTPrivateKeyParameters) {
                    ...
                    for (int r6 = 1; r6 < this.params.getLayers(); r6++) {
                        ...
                        build.getReducedSignatures().add(new XMSSReducedSignature.Builder(this.xmssParams).withWOTSPlusSignature(wotsSign).withAuthPath(bDSState.get(r6).getAuthenticationPath()).build());
                    }
    ...
    public boolean verifySignature(byte[] bArr, byte[] bArr2) {
        ...
        int r1 = 1;
        while (r1 < this.params.getLayers()) {
            int leafIndex2 = XMSSUtil.getLeafIndex(treeIndex, height);
            long treeIndex2 = XMSSUtil.getTreeIndex(treeIndex, height);
            OTSHashAddress oTSHashAddress2 = (OTSHashAddress) new OTSHashAddress.Builder().withLayerAddress(r1).withTreeAddress(treeIndex2).withOTSAddress(leafIndex2).build();
            rootNodeFromSignature = XMSSVerifierUtil.getRootNodeFromSignature(this.wotsPlus, height, rootNodeFromSignature.getValue(), build.getReducedSignatures().get(r1), oTSHashAddress2, leafIndex2);
            r1++;
