...
public class ExceptionUtils {
    ...
    public static int indexOfThrowable(Throwable th, Class<?> cls, int r3) {
        return indexOf(th, cls, r3, false);
    }
    ...
    public static int indexOfType(Throwable th, Class<?> cls, int r3) {
        return indexOf(th, cls, r3, true);
    }

    private static int indexOf(Throwable th, Class<?> cls, int r4, boolean z) {
        if (th != null && cls != null) {
            if (r4 < 0) {
                r4 = 0;
            }
            Throwable[] throwables = getThrowables(th);
            if (r4 >= throwables.length) {
                return -1;
            }
            if (z) {
                while (r4 < throwables.length) {
                    if (cls.isAssignableFrom(throwables[r4].getClass())) {
                        return r4;
                    }
                    r4++;
                }
                ...
                while (r4 < throwables.length) {
                    if (cls.equals(throwables[r4].getClass())) {
                        return r4;
                    }
                    r4++;
                }
    ...
    public static String[] getRootCauseStackTrace(Throwable th) {
        ...
        ArrayList arrayList = new ArrayList();
        int r2 = length - 1;
        List<String> stackFrameList = getStackFrameList(throwables[r2]);
        while (true) {
            length--;
            if (length >= 0) {
                if (length != 0) {
