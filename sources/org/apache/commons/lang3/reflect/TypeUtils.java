...
public class TypeUtils {
    ...
    private static Map<TypeVariable<?>, Type> getTypeArguments(Type type, Class<?> cls, Map<TypeVariable<?>, Type> map) {
        ...
        if (type instanceof WildcardType) {
            ...
            while (r2 < length) {
                Type type2 = implicitUpperBounds[r2];
                if (isAssignable(type2, cls)) {
    ...
    private static Map<TypeVariable<?>, Type> getTypeArguments(ParameterizedType parameterizedType, Class<?> cls, Map<TypeVariable<?>, Type> map) {
        ...
        if (isAssignable((Type) rawType, cls)) {
            ...
            for (int r2 = 0; r2 < typeParameters.length; r2++) {
                Type type = actualTypeArguments[r2];
                TypeVariable<Class<?>> typeVariable = typeParameters[r2];
    ...
    private static <T> void mapTypeVariablesToArguments(Class<T> cls, ParameterizedType parameterizedType, Map<TypeVariable<?>, Type> map) {
        ...
        for (int r1 = 0; r1 < actualTypeArguments.length; r1++) {
            TypeVariable<Class<?>> typeVariable = typeParameters[r1];
            Type type = actualTypeArguments[r1];
    ...
    public static Type unrollVariables(Map<TypeVariable<?>, Type> map, Type type) {
        ...
        if (containsTypeVariables(type)) {
            ...
            if (type instanceof ParameterizedType) {
                ...
                Type[] actualTypeArguments = parameterizedType.getActualTypeArguments();
                for (int r1 = 0; r1 < actualTypeArguments.length; r1++) {
                    Type unrollVariables = unrollVariables(map, actualTypeArguments[r1]);
                    if (unrollVariables != null) {
                        actualTypeArguments[r1] = unrollVariables;
                    }
                }
    ...
    private static Type[] unrollBounds(Map<TypeVariable<?>, Type> map, Type[] typeArr) {
        int r0 = 0;
        while (r0 < typeArr.length) {
            Type unrollVariables = unrollVariables(map, typeArr[r0]);
            if (unrollVariables == null) {
                typeArr = (Type[]) ArrayUtils.remove((Object[]) typeArr, r0);
                r0--;
            } else {
                typeArr[r0] = unrollVariables;
            }
            r0++;
        }
        return typeArr;
    ...
    private static boolean equals(Type[] typeArr, Type[] typeArr2) {
        if (typeArr.length == typeArr2.length) {
            for (int r0 = 0; r0 < typeArr.length; r0++) {
                if (!equals(typeArr[r0], typeArr2[r0])) {
                    return false;
                }
            }
            return true;
    ...
    private static void appendRecursiveTypes(StringBuilder sb, int[] r7, Type[] typeArr) {
        for (int r1 = 0; r1 < r7.length; r1++) {
            sb.append(Typography.less);
            appendAllTo(sb, ", ", typeArr[r1].toString()).append(Typography.greater);
        }
        Type[] typeArr2 = (Type[]) ArrayUtils.removeAll((Object[]) typeArr, r7);
    ...
    private static int[] findRecursiveTypes(ParameterizedType parameterizedType) {
        ...
        for (int r1 = 0; r1 < typeArr.length; r1++) {
            if ((typeArr[r1] instanceof TypeVariable) && containsVariableTypeSameParametrizedTypeBound((TypeVariable) typeArr[r1], parameterizedType)) {
                r2 = ArrayUtils.add(r2, r1);
            }
        }
        return r2;
    }
