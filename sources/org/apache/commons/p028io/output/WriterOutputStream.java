...
public class WriterOutputStream extends OutputStream {
    ...
    public WriterOutputStream(Writer writer, CharsetDecoder charsetDecoder, int r4, boolean z) {
        this.decoderIn = ByteBuffer.allocate(128);
        checkIbmJdkWithBrokenUTF16(charsetDecoder.charset());
        this.writer = writer;
        this.decoder = charsetDecoder;
        this.writeImmediately = z;
        this.decoderOut = CharBuffer.allocate(r4);
    }

    public WriterOutputStream(Writer writer, Charset charset, int r4, boolean z) {
        this(writer, charset.newDecoder().onMalformedInput(CodingErrorAction.REPLACE).onUnmappableCharacter(CodingErrorAction.REPLACE).replaceWith("?"), r4, z);
    }
    ...
    public WriterOutputStream(Writer writer, String str, int r3, boolean z) {
        this(writer, Charset.forName(str), r3, z);
    }
    ...
    public void write(byte[] bArr, int r4, int r5) throws IOException {
        while (r5 > 0) {
            int min = Math.min(r5, this.decoderIn.remaining());
            this.decoderIn.put(bArr, r4, min);
            processInput(false);
            r5 -= min;
            r4 += min;
        }
    ...
    private static void checkIbmJdkWithBrokenUTF16(Charset charset) {
        if ("UTF-16".equals(charset.name())) {
            ...
            int r6 = 0;
            while (r6 < length) {
                allocate.put(bytes[r6]);
                allocate.flip();
                try {
                    newDecoder.decode(allocate, allocate2, r6 == length + (-1));
                    allocate.compact();
                    r6++;
