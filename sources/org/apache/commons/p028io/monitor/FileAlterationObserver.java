...
public class FileAlterationObserver implements Serializable {
    ...
    private void checkAndNotify(FileEntry fileEntry, FileEntry[] fileEntryArr, File[] fileArr) {
        ...
        for (FileEntry fileEntry2 : fileEntryArr) {
            while (r3 < fileArr.length && this.comparator.compare(fileEntry2.getFile(), fileArr[r3]) > 0) {
                fileEntryArr2[r3] = createFileEntry(fileEntry, fileArr[r3]);
                doCreate(fileEntryArr2[r3]);
                r3++;
            }
            if (r3 < fileArr.length && this.comparator.compare(fileEntry2.getFile(), fileArr[r3]) == 0) {
                doMatch(fileEntry2, fileArr[r3]);
                checkAndNotify(fileEntry2, fileEntry2.getChildren(), listFiles(fileArr[r3]));
                fileEntryArr2[r3] = fileEntry2;
                r3++;
            } else {
                checkAndNotify(fileEntry2, fileEntry2.getChildren(), FileUtils.EMPTY_FILE_ARRAY);
                doDelete(fileEntry2);
    ...
    private FileEntry[] doListFiles(File file, FileEntry fileEntry) {
        ...
        for (int r1 = 0; r1 < listFiles.length; r1++) {
            fileEntryArr[r1] = createFileEntry(fileEntry, listFiles[r1]);
        }
        return fileEntryArr;
    }
