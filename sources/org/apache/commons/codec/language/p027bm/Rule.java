...
public class Rule {
    ...
    public static final class Phoneme implements PhonemeExpr {
        public static final Comparator<Phoneme> COMPARATOR = new Comparator<Phoneme>() { // from class: org.apache.commons.codec.language.bm.Rule.Phoneme.1
            ...
            public int compare(Phoneme phoneme, Phoneme phoneme2) {
                for (int r1 = 0; r1 < phoneme.phonemeText.length(); r1++) {
                    if (r1 >= phoneme2.phonemeText.length()) {
                        return 1;
                    }
                    int charAt = phoneme.phonemeText.charAt(r1) - phoneme2.phonemeText.charAt(r1);
                    if (charAt != 0) {
                        return charAt;
                    }
                }
    ...
    private static Map<String, List<Rule>> parseRules(Scanner scanner, String str) {
        ...
        while (scanner.hasNextLine()) {
            ...
            if (z) {
                ...
                String trim = (indexOf >= 0 ? nextLine.substring(r12, indexOf) : nextLine).trim();
                if (trim.length() == 0) {
                    r1 = r15;
                    ...
                    try {
                        stripQuotes = stripQuotes(split[r12]);
                    ...
                    try {
                        Rule rule = new Rule(stripQuotes, stripQuotes2, stripQuotes3, parsePhonemeExpr(stripQuotes(split[3])), r15, str, stripQuotes, stripQuotes2, stripQuotes3) { // from class: org.apache.commons.codec.language.bm.Rule.2
                        ...
                        e = e2;
                        throw new IllegalStateException("Problem parsing line '" + r15 + str2 + str, e);
                    }
                }
            }
            r1 = r15;
    ...
    public static boolean startsWith(CharSequence charSequence, CharSequence charSequence2) {
        ...
        for (int r0 = 0; r0 < charSequence2.length(); r0++) {
            if (charSequence.charAt(r0) != charSequence2.charAt(r0)) {
                return false;
            }
        }
        return true;
    ...
    public boolean patternAndContextMatches(CharSequence charSequence, int r6) {
        if (r6 < 0) {
            throw new IndexOutOfBoundsException("Can not match pattern at negative indexes");
        }
        int length = this.pattern.length() + r6;
        if (length <= charSequence.length() && charSequence.subSequence(r6, length).equals(this.pattern) && this.rContext.isMatch(charSequence.subSequence(length, charSequence.length()))) {
            return this.lContext.isMatch(charSequence.subSequence(0, r6));
        }
        return false;
    }
}
