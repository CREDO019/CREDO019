...
public class MemberComparator implements Comparator<DeclarationDescriptor> {
    ...
    public int compare(DeclarationDescriptor declarationDescriptor, DeclarationDescriptor declarationDescriptor2) {
        ...
        if ((declarationDescriptor instanceof TypeAliasDescriptor) && (declarationDescriptor2 instanceof TypeAliasDescriptor)) {
            ...
            for (int r5 = 0; r5 < Math.min(valueParameters.size(), valueParameters2.size()); r5++) {
                DescriptorRenderer descriptorRenderer3 = RENDERER;
                int compareTo3 = descriptorRenderer3.renderType(valueParameters.get(r5).getType()).compareTo(descriptorRenderer3.renderType(valueParameters2.get(r5).getType()));
                if (compareTo3 != 0) {
                    return compareTo3;
                }
            }
            ...
            for (int r52 = 0; r52 < Math.min(typeParameters.size(), typeParameters2.size()); r52++) {
                List<KotlinType> upperBounds = typeParameters.get(r52).getUpperBounds();
                List<KotlinType> upperBounds2 = typeParameters2.get(r52).getUpperBounds();
                int size2 = upperBounds.size() - upperBounds2.size();
                if (size2 != 0) {
                    return size2;
                }
                for (int r8 = 0; r8 < upperBounds.size(); r8++) {
                    DescriptorRenderer descriptorRenderer4 = RENDERER;
                    int compareTo4 = descriptorRenderer4.renderType(upperBounds.get(r8)).compareTo(descriptorRenderer4.renderType(upperBounds2.get(r8)));
                    if (compareTo4 != 0) {
                        return compareTo4;
                    }
                }
