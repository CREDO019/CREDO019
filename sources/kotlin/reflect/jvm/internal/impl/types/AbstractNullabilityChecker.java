                ...
                if (supertypesSet.add(current)) {
                    ...
                    if (none == null) {
                        ...
                        for (KotlinTypeMarker kotlinTypeMarker : typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current))) {
                            SimpleTypeMarker mo3015transformType = none.mo3015transformType(typeCheckerState, kotlinTypeMarker);
                            if ((typeSystemContext.isClassType(mo3015transformType) && !typeSystemContext.isMarkedNullable(mo3015transformType)) || typeSystemContext.isDefinitelyNotNullType(mo3015transformType)) {
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    for (KotlinTypeMarker kotlinTypeMarker : typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current))) {
                        SimpleTypeMarker mo3015transformType = supertypesPolicy.mo3015transformType(state, kotlinTypeMarker);
                        if (INSTANCE.isApplicableAsEndNode(state, mo3015transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(mo3015transformType);
