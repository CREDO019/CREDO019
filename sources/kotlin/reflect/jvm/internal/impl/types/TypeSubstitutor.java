...
public class TypeSubstitutor {
    ...
    */
    ...
    private TypeProjection unsafeSubstitute(TypeProjection typeProjection, TypeParameterDescriptor typeParameterDescriptor, int r10) throws SubstitutionException {
        KotlinType makeNullableIfNeeded;
        if (typeProjection == null) {
            $$$reportNull$$$0(18);
        }
        assertRecursionDepth(r10, typeProjection, this.substitution);
        ...
        if (type instanceof TypeWithEnhancement) {
            ...
            TypeProjection unsafeSubstitute = unsafeSubstitute(new TypeProjectionImpl(typeProjection.getProjectionKind(), origin), typeParameterDescriptor, r10 + 1);
            ...
            TypeProjection mo3016get = this.substitution.mo3016get(type);
            TypeProjection projectedTypeForConflictedTypeWithUnsafeVariance = mo3016get != null ? projectedTypeForConflictedTypeWithUnsafeVariance(type, mo3016get, typeParameterDescriptor, typeProjection) : null;
            ...
            if (projectedTypeForConflictedTypeWithUnsafeVariance == null && FlexibleTypesKt.isFlexible(type) && !TypeCapabilitiesKt.isCustomTypeVariable(type)) {
                ...
                int r102 = r10 + 1;
                TypeProjection unsafeSubstitute2 = unsafeSubstitute(new TypeProjectionImpl(projectionKind, asFlexibleType.getLowerBound()), typeParameterDescriptor, r102);
                TypeProjection unsafeSubstitute3 = unsafeSubstitute(new TypeProjectionImpl(projectionKind, asFlexibleType.getUpperBound()), typeParameterDescriptor, r102);
                ...
                if (!CapturedTypeConstructorKt.isCaptured(type)) {
                    int r9 = C48982.f1532x4790888d[conflictType.ordinal()];
                    if (r9 == 1) {
                ...
                TypeCapabilities customTypeVariable = TypeCapabilitiesKt.getCustomTypeVariable(type);
                ...
                TypeProjection substituteCompoundType = substituteCompoundType(typeProjection, r10);
    ...
    /* loaded from: classes5.dex */
    public static /* synthetic */ class C48982 {

        /* renamed from: $SwitchMap$org$jetbrains$kotlin$types$TypeSubstitutor$VarianceConflictType */
        static final /* synthetic */ int[] f1532x4790888d;

        static {
            int[] r0 = new int[VarianceConflictType.values().length];
            f1532x4790888d = r0;
            try {
                r0[VarianceConflictType.OUT_IN_IN_POSITION.ordinal()] = 1;
            } catch (NoSuchFieldError unused) {
            }
            try {
                f1532x4790888d[VarianceConflictType.IN_IN_OUT_POSITION.ordinal()] = 2;
            } catch (NoSuchFieldError unused2) {
            }
            try {
                f1532x4790888d[VarianceConflictType.NO_CONFLICT.ordinal()] = 3;
    ...
    private TypeProjection substituteCompoundType(TypeProjection typeProjection, int r6) throws SubstitutionException {
        KotlinType type = typeProjection.getType();
        Variance projectionKind = typeProjection.getProjectionKind();
        if (type.getConstructor().mo3011getDeclarationDescriptor() instanceof TypeParameterDescriptor) {
            return typeProjection;
        }
        ...
        KotlinType replace = TypeSubstitutionKt.replace(type, substituteTypeArguments(type.getConstructor().getParameters(), type.getArguments(), r6), this.substitution.filterAnnotations(type.getAnnotations()));
    ...
    private List<TypeProjection> substituteTypeArguments(List<TypeParameterDescriptor> list, List<TypeProjection> list2, int r13) throws SubstitutionException {
        ArrayList arrayList = new ArrayList(list.size());
        boolean z = false;
        for (int r1 = 0; r1 < list.size(); r1++) {
            TypeParameterDescriptor typeParameterDescriptor = list.get(r1);
            TypeProjection typeProjection = list2.get(r1);
            TypeProjection unsafeSubstitute = unsafeSubstitute(typeProjection, typeParameterDescriptor, r13 + 1);
            int r7 = C48982.f1532x4790888d[conflictType(typeParameterDescriptor.getVariance(), unsafeSubstitute.getProjectionKind()).ordinal()];
            if (r7 != 1 && r7 != 2) {
                if (r7 == 3 && typeParameterDescriptor.getVariance() != Variance.INVARIANT && !unsafeSubstitute.isStarProjection()) {
    ...
    private static void assertRecursionDepth(int r2, TypeProjection typeProjection, TypeSubstitution typeSubstitution) {
        if (r2 <= 100) {
            return;
        }
