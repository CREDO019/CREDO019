...
public final class descriptorBasedTypeSignatureMapping {
    public static /* synthetic */ Object mapType$default(KotlinType kotlinType, JvmTypeFactory jvmTypeFactory, TypeMappingMode typeMappingMode, TypeMappingConfiguration typeMappingConfiguration, typeSignatureMapping typesignaturemapping, Function3 function3, int r12, Object obj) {
        if ((r12 & 32) != 0) {
    ...
    public static final <T> T mapType(KotlinType kotlinType, JvmTypeFactory<T> factory, TypeMappingMode mode, TypeMappingConfiguration<? extends T> typeMappingConfiguration, typeSignatureMapping<T> typesignaturemapping, Function3<? super KotlinType, ? super T, ? super TypeMappingMode, Unit> writeGenericType) {
        ...
        if (constructor instanceof IntersectionTypeConstructor) {
            ...
            if (alternativeType == null) {
                alternativeType = typeMappingConfiguration.commonSupertype(intersectionTypeConstructor.mo3012getSupertypes());
            }
        ...
        ClassifierDescriptor classifierDescriptor = mo3011getDeclarationDescriptor;
        if (ErrorUtils.isError(classifierDescriptor)) {
            T t = (T) factory.createObjectType("error/NonExistentClass");
            typeMappingConfiguration.processErrorType(kotlinType, (ClassDescriptor) mo3011getDeclarationDescriptor);
            if (typesignaturemapping != 0) {
                typesignaturemapping.writeClass(t);
            }
            return t;
        }
        boolean z = mo3011getDeclarationDescriptor instanceof ClassDescriptor;
        if (z && KotlinBuiltIns.isArray(kotlinType)) {
            ...
            if (mode.isForAnnotationParameter() && KotlinBuiltIns.isKClass((ClassDescriptor) mo3011getDeclarationDescriptor)) {
            ...
            T t2 = (T) mapType(TypeUtils.getRepresentativeUpperBound((TypeParameterDescriptor) mo3011getDeclarationDescriptor), factory, mode, typeMappingConfiguration, null, functions.getDO_NOTHING_3());
            if (typesignaturemapping != 0) {
                Name name = mo3011getDeclarationDescriptor.getName();
                Intrinsics.checkNotNullExpressionValue(name, "descriptor.getName()");
                typesignaturemapping.writeTypeVariable(name, t2);
            }
            return t2;
        } else if ((mo3011getDeclarationDescriptor instanceof TypeAliasDescriptor) && mode.getMapTypeAliases()) {
            return (T) mapType(((TypeAliasDescriptor) mo3011getDeclarationDescriptor).getExpandedType(), factory, mode, typeMappingConfiguration, typesignaturemapping, writeGenericType);
        } else {
    ...
    public static /* synthetic */ String computeInternalName$default(ClassDescriptor classDescriptor, TypeMappingConfiguration typeMappingConfiguration, int r2, Object obj) {
        if ((r2 & 2) != 0) {
