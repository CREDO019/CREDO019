...
public final class JavaTypeResolver {
    ...
    /* renamed from: c */
    private final LazyJavaResolverContext f1515c;
    ...
    public final KotlinType transformJavaType(JavaType javaType, JavaTypeAttributes attr) {
        ...
        if (javaType instanceof JavaPrimitiveType) {
            ...
            if (type != null) {
                unitType = this.f1515c.getModule().getBuiltIns().getPrimitiveKotlinType(type);
            } else {
                unitType = this.f1515c.getModule().getBuiltIns().getUnitType();
            }
            ...
            if (!(javaType instanceof JavaWildcardType)) {
                if (javaType == null) {
                    SimpleType defaultBound = this.f1515c.getModule().getBuiltIns().getDefaultBound();
            ...
            if (bound == null) {
                SimpleType defaultBound2 = this.f1515c.getModule().getBuiltIns().getDefaultBound();
    ...
    public static /* synthetic */ KotlinType transformArrayType$default(JavaTypeResolver javaTypeResolver, javaTypes javatypes, JavaTypeAttributes javaTypeAttributes, boolean z, int r4, Object obj) {
        if ((r4 & 4) != 0) {
            z = false;
        }
    ...
    public final KotlinType transformArrayType(javaTypes arrayType, JavaTypeAttributes attr, boolean z) {
        ...
        LazyJavaAnnotations lazyJavaAnnotations = new LazyJavaAnnotations(this.f1515c, arrayType, true);
        if (type != null) {
            SimpleType primitiveArrayKotlinType = this.f1515c.getModule().getBuiltIns().getPrimitiveArrayKotlinType(type);
        ...
        if (attr.isForAnnotationParameter()) {
            SimpleType arrayType2 = this.f1515c.getModule().getBuiltIns().getArrayType(z ? Variance.OUT_VARIANCE : Variance.INVARIANT, transformJavaType, lazyJavaAnnotations);
        ...
        SimpleType arrayType3 = this.f1515c.getModule().getBuiltIns().getArrayType(Variance.INVARIANT, transformJavaType, lazyJavaAnnotations2);
        Intrinsics.checkNotNullExpressionValue(arrayType3, "c.module.builtIns.getArr…mponentType, annotations)");
        return KotlinTypeFactory.flexibleType(arrayType3, this.f1515c.getModule().getBuiltIns().getArrayType(Variance.OUT_VARIANCE, transformJavaType, lazyJavaAnnotations2).makeNullableAsSpecified(true));
    }
    ...
    private final SimpleType computeSimpleJavaClassifierType(JavaClassifierType javaClassifierType, JavaTypeAttributes javaTypeAttributes, SimpleType simpleType) {
        LazyJavaAnnotations lazyJavaAnnotations = simpleType == null ? new LazyJavaAnnotations(this.f1515c, javaClassifierType, false, 4, null) : simpleType.getAnnotations();
    ...
    private final TypeConstructor computeTypeConstructor(JavaClassifierType javaClassifierType, JavaTypeAttributes javaTypeAttributes) {
        ...
        if (classifier instanceof JavaClass) {
            ...
            if (mapKotlinClass == null) {
                mapKotlinClass = this.f1515c.getComponents().getModuleClassResolver().resolveClass(javaClass);
            }
    ...
    private final TypeConstructor createNotFoundClass(JavaClassifierType javaClassifierType) {
        ...
        TypeConstructor typeConstructor = this.f1515c.getComponents().getDeserializedDescriptorResolver().getComponents().getNotFoundClasses().getClass(classId, CollectionsKt.listOf(0)).getTypeConstructor();
    ...
    private final ClassDescriptor mapKotlinClass(JavaClassifierType javaClassifierType, JavaTypeAttributes javaTypeAttributes, FqName fqName) {
        if (javaTypeAttributes.isForAnnotationParameter() && Intrinsics.areEqual(fqName, JavaTypeResolverKt.access$getJAVA_LANG_CLASS_FQ_NAME$p())) {
            return this.f1515c.getComponents().getReflectionTypes().getKClass();
        }
        JavaToKotlinClassMapper javaToKotlinClassMapper = JavaToKotlinClassMapper.INSTANCE;
        ClassDescriptor mapJavaToKotlin$default = JavaToKotlinClassMapper.mapJavaToKotlin$default(javaToKotlinClassMapper, fqName, this.f1515c.getModule().getBuiltIns(), null, 4, null);
    ...
    private final List<TypeProjection> computeRawTypeArguments(final JavaClassifierType javaClassifierType, List<? extends TypeParameterDescriptor> list, final TypeConstructor typeConstructor, final JavaTypeAttributes javaTypeAttributes) {
        ...
        for (final TypeParameterDescriptor typeParameterDescriptor : list2) {
            if (TypeUtils.hasTypeParameterRecursiveBounds(typeParameterDescriptor, null, javaTypeAttributes.getVisitedTypeParameters())) {
                ...
                computeProjection = this.rawSubstitution.computeProjection(typeParameterDescriptor, javaClassifierType.isRaw() ? javaTypeAttributes : javaTypeAttributes.withFlexibility(JavaTypeFlexibility.INFLEXIBLE), new LazyWrappedType(this.f1515c.getStorageManager(), new Functions<KotlinType>() { // from class: kotlin.reflect.jvm.internal.impl.load.java.lazy.types.JavaTypeResolver$computeRawTypeArguments$1$erasedUpperBound$1
                    ...
                    public final KotlinType invoke() {
                        ...
                        KotlinType erasedUpperBound$descriptors_jvm = typeParameterUpperBoundEraser.getErasedUpperBound$descriptors_jvm(typeParameterDescriptor2, isRaw, javaTypeAttributes2.withDefaultType(mo3011getDeclarationDescriptor == null ? null : mo3011getDeclarationDescriptor.getDefaultType()));
