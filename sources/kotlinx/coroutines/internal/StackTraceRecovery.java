...
public final class StackTraceRecovery {
    ...
    static {
        ...
        try {
            ...
            obj = Result.m1749constructorimpl(Class.forName(baseContinuationImplClass).getCanonicalName());
        } catch (Throwable th) {
            Result.Companion companion2 = Result.Companion;
            obj = Result.m1749constructorimpl(ResultKt.createFailure(th));
        }
        if (Result.m1752exceptionOrNullimpl(obj) == null) {
            obj4 = obj;
        }
        baseContinuationImplClassName = (String) obj4;
        try {
            Result.Companion companion3 = Result.Companion;
            obj2 = Result.m1749constructorimpl(Class.forName(stackTraceRecoveryClass).getCanonicalName());
        } catch (Throwable th2) {
            Result.Companion companion4 = Result.Companion;
            obj2 = Result.m1749constructorimpl(ResultKt.createFailure(th2));
        }
        if (Result.m1752exceptionOrNullimpl(obj2) == null) {
            obj3 = obj2;
        }
    ...
    private static final <E extends Throwable> E sanitizeStackTrace(E e) {
        ...
        int r1 = (length - frameIndex) - (frameIndex2 == -1 ? 0 : length - frameIndex2);
        StackTraceElement[] stackTraceElementArr = new StackTraceElement[r1];
        for (int r5 = 0; r5 < r1; r5++) {
            if (r5 == 0) {
                stackTraceElement = artificialFrame("Coroutine boundary");
            } else {
                stackTraceElement = stackTrace[(r3 + r5) - 1];
            }
            stackTraceElementArr[r5] = stackTraceElement;
        }
    ...
    private static final <E extends Throwable> E createFinalException(E e, E e2, ArrayDeque<StackTraceElement> arrayDeque) {
        ...
        if (frameIndex != -1) {
            ...
            for (int r3 = 0; r3 < frameIndex; r3++) {
                stackTraceElementArr[r3] = stackTrace[r3];
            }
            Iterator<StackTraceElement> it = arrayDeque.iterator();
            while (it.hasNext()) {
                int r7 = r1 + 1;
                stackTraceElementArr[r1 + frameIndex] = it.next();
                r1 = r7;
            }
        ...
        if (cause != null && Intrinsics.areEqual(cause.getClass(), e.getClass())) {
            ...
            while (true) {
                if (r4 >= length) {
                    z = false;
                    break;
                } else if (isArtificial(stackTrace[r4])) {
                    z = true;
                    break;
                } else {
                    r4++;
            ...
            if (z) {
                return TuplesKt.m176to(cause, stackTrace);
            }
            return TuplesKt.m176to(e, new StackTraceElement[0]);
        }
        return TuplesKt.m176to(e, new StackTraceElement[0]);
    }
    ...
    private static final void mergeRecoveredTraces(StackTraceElement[] stackTraceElementArr, ArrayDeque<StackTraceElement> arrayDeque) {
        ...
        while (true) {
            if (r1 >= length) {
                r1 = -1;
                break;
            } else if (isArtificial(stackTraceElementArr[r1])) {
                break;
            } else {
                r1++;
            }
    ...
    private static final int frameIndex(StackTraceElement[] stackTraceElementArr, String str) {
        ...
        for (int r1 = 0; r1 < length; r1++) {
            if (Intrinsics.areEqual(str, stackTraceElementArr[r1].getClassName())) {
                return r1;
            }
        }
        return -1;
