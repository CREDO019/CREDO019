...
public final class RealBufferedSource implements BufferedSource {
    ...
    public InputStream inputStream() {
        return new InputStream() { // from class: okio.RealBufferedSource$inputStream$1
            ...
            @Override // java.io.InputStream
            public int read(byte[] data, int r9, int r10) {
                Intrinsics.checkNotNullParameter(data, "data");
                if (RealBufferedSource.this.closed) {
                    throw new IOException(ReactNativeGoogleMobileAdsEvent.GOOGLE_MOBILE_ADS_EVENT_CLOSED);
                }
                Util.checkOffsetAndCount(data.length, r9, r10);
                if (RealBufferedSource.this.bufferField.size() == 0 && RealBufferedSource.this.source.read(RealBufferedSource.this.bufferField, 8192) == -1) {
                    return -1;
                }
                return RealBufferedSource.this.bufferField.read(data, r9, r10);
            }
    ...
    public void readFully(byte[] sink) {
        ...
        try {
            ...
            while (this.bufferField.size() > 0) {
                ...
                int read = buffer.read(sink, r1, (int) buffer.size());
                if (read == -1) {
    ...
    @Override // okio.BufferedSource
    public int read(byte[] sink, int r11, int r12) {
        Intrinsics.checkNotNullParameter(sink, "sink");
        long j = r12;
        ...
        return this.bufferField.read(sink, r11, (int) Math.min(j, this.bufferField.size()));
    }
    ...
    */
    ...
    @Override // okio.BufferedSource
    public boolean rangeEquals(long j, ByteString bytes, int r11, int r12) {
        int r2;
        Intrinsics.checkNotNullParameter(bytes, "bytes");
        if (!(!this.closed)) {
            throw new IllegalStateException(ReactNativeGoogleMobileAdsEvent.GOOGLE_MOBILE_ADS_EVENT_CLOSED.toString());
        ...
        if (j >= 0 && r11 >= 0 && r12 >= 0 && bytes.size() - r11 >= r12) {
            while (r2 < r12) {
                long j2 = r2 + j;
                r2 = (request(1 + j2) && this.bufferField.getByte(j2) == bytes.getByte(r11 + r2)) ? r2 + 1 : 0;
            }
            return true;
        }
        return false;
