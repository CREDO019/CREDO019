...
public final class StandardMutableNetwork<N, E> extends StandardNetwork<N, E> implements MutableNetwork<N, E> {
    ...
    public boolean addEdge(N n, N n2, E e) {
        ...
        if (containsEdge(e)) {
            ...
            EndpointPair m295of = EndpointPair.m295of(this, n, n2);
            Preconditions.checkArgument(incidentNodes.equals(m295of), "Edge %s already exists between the following nodes: %s, so it cannot be reused to connect the following nodes: %s.", e, incidentNodes, m295of);
            return false;
        }
    ...
    private NetworkConnections<N, E> newConnections() {
        if (isDirected()) {
            if (allowsParallelEdges()) {
                return DirectedMultiNetworkConnections.m298of();
            }
            return DirectedNetworkConnections.m297of();
        } else if (allowsParallelEdges()) {
            return UndirectedMultiNetworkConnections.m292of();
        } else {
            return UndirectedNetworkConnections.m291of();
        }
    }
}
