...
public final class Iterables {
            ...
            if (!predicate.apply(t)) {
                if (r1 > r2) {
                    try {
                        ...
                        slowRemoveIfForRemainingElements(list, predicate, r2, r1);
                        return true;
    ...
    private static <T> void slowRemoveIfForRemainingElements(List<T> list, Predicate<? super T> predicate, int r4, int r5) {
        for (int size = list.size() - 1; size > r5; size--) {
            if (predicate.apply(list.get(size))) {
                list.remove(size);
            }
        }
        for (int r52 = r5 - 1; r52 >= r4; r52--) {
            list.remove(r52);
        }
    }
    ...
    public static <T> Iterable<List<T>> partition(final Iterable<T> iterable, final int r2) {
        Preconditions.checkNotNull(iterable);
        Preconditions.checkArgument(r2 > 0);
        return new FluentIterable<List<T>>() { // from class: com.google.common.collect.Iterables.2
            @Override // java.lang.Iterable
            public Iterator<List<T>> iterator() {
                return Iterators.partition(iterable.iterator(), r2);
            }
        };
    }

    public static <T> Iterable<List<T>> paddedPartition(final Iterable<T> iterable, final int r2) {
        Preconditions.checkNotNull(iterable);
        Preconditions.checkArgument(r2 > 0);
        return new FluentIterable<List<T>>() { // from class: com.google.common.collect.Iterables.3
            @Override // java.lang.Iterable
            public Iterator<List<T>> iterator() {
                return Iterators.paddedPartition(iterable.iterator(), r2);
            }
        };
    }
    ...
    @ParametricNullness
    public static <T> T get(Iterable<T> iterable, int r2) {
        Preconditions.checkNotNull(iterable);
        if (iterable instanceof List) {
            return (T) ((List) iterable).get(r2);
        }
        return (T) Iterators.get(iterable.iterator(), r2);
    }

    @ParametricNullness
    public static <T> T get(Iterable<? extends T> iterable, int r2, @ParametricNullness T t) {
        Preconditions.checkNotNull(iterable);
        Iterators.checkNonnegative(r2);
        if (iterable instanceof List) {
            List cast = Lists.cast(iterable);
            return r2 < cast.size() ? (T) cast.get(r2) : t;
        }
    ...
    public static <T> Iterable<T> skip(final Iterable<T> iterable, final int r3) {
        Preconditions.checkNotNull(iterable);
        Preconditions.checkArgument(r3 >= 0, "number to skip cannot be negative");
    ...
    public static <T> Iterable<T> limit(final Iterable<T> iterable, final int r3) {
        Preconditions.checkNotNull(iterable);
        ...
        return new FluentIterable<T>() { // from class: com.google.common.collect.Iterables.7
            @Override // java.lang.Iterable
            public Iterator<T> iterator() {
                return Iterators.limit(iterable.iterator(), r3);
            }
        };
    }
