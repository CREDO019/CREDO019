...
public final class HostAndPort implements Serializable {
    ...
    private static boolean isValidPort(int r1) {
        return r1 >= 0 && r1 <= 65535;
    }

    private HostAndPort(String str, int r2, boolean z) {
        this.host = str;
        this.port = r2;
    ...
    public int getPortOrDefault(int r2) {
        return hasPort() ? this.port : r2;
    }

    public static HostAndPort fromParts(String str, int r4) {
        Preconditions.checkArgument(isValidPort(r4), "Port out of range: %s", r4);
        HostAndPort fromString = fromString(str);
        Preconditions.checkArgument(!fromString.hasPort(), "Host has a port: %s", str);
        return new HostAndPort(fromString.host, r4, fromString.hasBracketlessColons);
    }
    ...
    */
    ...
    private static String[] getHostAndPortFromBracketedHost(String str) {
        ...
        int r5 = lastIndexOf + 1;
        if (r5 == str.length()) {
            return new String[]{substring, ""};
        }
        Preconditions.checkArgument(str.charAt(r5) == ':', "Only a colon may follow a close bracket: %s", str);
        int r4 = lastIndexOf + 2;
        for (int r1 = r4; r1 < str.length(); r1++) {
            Preconditions.checkArgument(Character.isDigit(str.charAt(r1)), "Port must be numeric: %s", str);
        }
        return new String[]{substring, str.substring(r4)};
    }

    public HostAndPort withDefaultPort(int r4) {
        Preconditions.checkArgument(isValidPort(r4));
        return hasPort() ? this : new HostAndPort(this.host, r4, this.hasBracketlessColons);
    }
