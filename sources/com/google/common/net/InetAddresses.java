...
import com.google.common.p016io.ByteStreams;
...
public final class InetAddresses {
    ...
    private static final CharMatcher IPV4_DELIMITER_MATCHER = CharMatcher.m443is('.');
    private static final char IPV6_DELIMITER = ':';
    private static final CharMatcher IPV6_DELIMITER_MATCHER = CharMatcher.m443is(IPV6_DELIMITER);
    ...
    */
    ...
    private static byte[] textToNumericFormatV4(String str) {
        ...
        for (int r3 = 0; r3 < 4; r3++) {
            int indexOf = str.indexOf(46, r4);
            if (indexOf == -1) {
                indexOf = str.length();
            }
            try {
                bArr[r3] = parseOctet(str, r4, indexOf);
                r4 = indexOf + 1;
    ...
    private static byte[] textToNumericFormatV6(String str) {
        ...
        if (countIn >= 2 && countIn <= 8) {
            ...
            int r5 = 8 - r0;
            boolean z = false;
            for (int r7 = 0; r7 < str.length() - 1; r7++) {
                if (str.charAt(r7) == ':' && str.charAt(r7 + 1) == ':') {
                    if (z) {
                        return null;
                    }
                    r5++;
            ...
            try {
                ...
                while (r4 < str.length()) {
                    int indexOf = str.indexOf(58, r4);
                    if (indexOf == -1) {
                        indexOf = str.length();
                    }
                    if (str.charAt(r4) == ':') {
                        for (int r3 = 0; r3 < r5; r3++) {
                            allocate.putShort((short) 0);
                        }
                    } else {
                        allocate.putShort(parseHextet(str, r4, indexOf));
                    }
                    r4 = indexOf + 1;
                }
    ...
    private static byte parseOctet(String str, int r4, int r5) {
        int r0 = r5 - r4;
        if (r0 <= 0 || r0 > 3) {
            throw new NumberFormatException();
        }
        if (r0 <= 1 || str.charAt(r4) != '0') {
            int r02 = 0;
            while (r4 < r5) {
                int r03 = r02 * 10;
                int digit = Character.digit(str.charAt(r4), 10);
                if (digit < 0) {
                    throw new NumberFormatException();
                }
                r02 = r03 + digit;
    ...
    private static short parseHextet(String str, int r4, int r5) {
        int r0 = r5 - r4;
        if (r0 <= 0 || r0 > 4) {
            throw new NumberFormatException();
        }
        int r02 = 0;
        while (r4 < r5) {
            r02 = (r02 << 4) | Character.digit(str.charAt(r4), 16);
            r4++;
        }
        return (short) r02;
    }
    ...
    public static String toAddrString(InetAddress inetAddress) {
        ...
        for (int r3 = 0; r3 < 8; r3++) {
            int r4 = r3 * 2;
            r1[r3] = Ints.fromBytes((byte) 0, (byte) 0, address[r4], address[r4 + 1]);
        }
        compressLongestRunOfZeroes(r1);
        return hextetsToIPv6String(r1);
    }

    private static void compressLongestRunOfZeroes(int[] r6) {
        int r2 = -1;
        int r3 = -1;
        int r4 = -1;
        for (int r1 = 0; r1 < r6.length + 1; r1++) {
            if (r1 >= r6.length || r6[r1] != 0) {
                if (r4 >= 0) {
                    int r5 = r1 - r4;
                    if (r5 > r2) {
                        r3 = r4;
        ...
        if (r2 >= 2) {
            Arrays.fill(r6, r3, r2 + r3, -1);
        }
    }

    private static String hextetsToIPv6String(int[] r5) {
        ...
        while (r2 < r5.length) {
            ...
            if (z2) {
                ...
                sb.append(Integer.toHexString(r5[r2]));
            } else if (r2 == 0 || z) {
                sb.append("::");
            }
            r2++;
    ...
    public static final class TeredoInfo {
        ...
        public TeredoInfo(@CheckForNull Inet4Address inet4Address, @CheckForNull Inet4Address inet4Address2, int r8, int r9) {
            boolean z = true;
            Preconditions.checkArgument(r8 >= 0 && r8 <= 65535, "port '%s' is out of range (0 <= port <= 0xffff)", r8);
            Preconditions.checkArgument((r9 < 0 || r9 > 65535) ? false : false, "flags '%s' is out of range (0 <= flags <= 0xffff)", r9);
    ...
    public static TeredoInfo getTeredoInfo(Inet6Address inet6Address) {
        ...
        int r2 = 65535 & (~ByteStreams.newDataInput(address, 10).readShort());
        byte[] copyOfRange = Arrays.copyOfRange(address, 12, 16);
        for (int r3 = 0; r3 < copyOfRange.length; r3++) {
            copyOfRange[r3] = (byte) (~copyOfRange[r3]);
        }
        return new TeredoInfo(inet4Address, getInet4Address(copyOfRange), r2, readShort);
    }
    ...
    public static boolean isMappedIPv4Address(String str) {
        ...
        while (true) {
            if (r1 >= 10) {
                for (int r2 = 10; r2 < 12; r2++) {
                    if (ipStringToBytes[r2] != -1) {
                        return false;
                    }
                }
                return true;
            } else if (ipStringToBytes[r1] != 0) {
                return false;
            } else {
                r1++;
            }
    ...
    public static Inet4Address fromInteger(int r0) {
        return getInet4Address(Ints.toByteArray(r0));
    }
    ...
    private static InetAddress fromBigInteger(BigInteger bigInteger, boolean z) {
        ...
        int max = Math.max(0, byteArray.length - r10);
        int length = byteArray.length - max;
        int r6 = r10 - length;
        for (int r7 = 0; r7 < max; r7++) {
            if (byteArray[r7] != 0) {
                throw formatIllegalArgumentException("BigInteger cannot be converted to InetAddress because it has more than %d bytes: %s", Integer.valueOf(r10), bigInteger);
            }
        }
        System.arraycopy(byteArray, max, bArr, r6, length);
        try {
            return InetAddress.getByAddress(bArr);
        } catch (UnknownHostException e) {
            throw new AssertionError(e);
    ...
    public static InetAddress fromLittleEndianByteArray(byte[] bArr) throws UnknownHostException {
        ...
        for (int r1 = 0; r1 < bArr.length; r1++) {
            bArr2[r1] = bArr[(bArr.length - r1) - 1];
        }
