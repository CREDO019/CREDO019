...
public final class InternetDomainName {
    ...
    private static final Splitter DOT_SPLITTER = Splitter.m431on('.');
    private static final Joiner DOT_JOINER = Joiner.m441on('.');
    ...
    static {
        ...
        CharMatcher mo442or = CharMatcher.inRange('a', 'z').mo442or(CharMatcher.inRange('A', Matrix.MATRIX_TYPE_ZERO));
        LETTER_MATCHER = mo442or;
        PART_CHAR_MATCHER = inRange.mo442or(mo442or).mo442or(anyOf);
    }
    ...
    InternetDomainName(String str) {
        ...
        this.registrySuffixIndex = findSuffixOfType(Optional.m439of(PublicSuffixType.REGISTRY));
    }

    private int findSuffixOfType(Optional<PublicSuffixType> optional) {
        int size = this.parts.size();
        for (int r1 = 0; r1 < size; r1++) {
            String join = DOT_JOINER.join(this.parts.subList(r1, size));
    ...
    private static boolean validateSyntax(List<String> list) {
        ...
        if (validatePart(list.get(size), true)) {
            for (int r2 = 0; r2 < size; r2++) {
                if (!validatePart(list.get(r2), false)) {
                    return false;
                }
            }
            return true;
    ...
    private InternetDomainName ancestor(int r4) {
        Joiner joiner = DOT_JOINER;
        ImmutableList<String> immutableList = this.parts;
        return from(joiner.join(immutableList.subList(r4, immutableList.size())));
    }
