...
public class GoogleApiManager implements Handler.Callback {
    ...
    private final void zaL(TaskCompletionSource taskCompletionSource, int r3, GoogleApi googleApi) {
        zacd zaa2;
        if (r3 == 0 || (zaa2 = zacd.zaa(this, r3, googleApi.getApiKey())) == null) {
            return;
        }
    ...
    public final boolean handleMessage(Message message) {
        ...
        switch (message.what) {
            ...
            case 5:
                ...
                while (true) {
                    if (it2.hasNext()) {
                        ...
                        zabqVar = zabqVar5.zab() == r0 ? zabqVar5 : null;
                    }
                }
                if (zabqVar == null) {
                    Log.wtf("GoogleApiManager", "Could not find API instance " + r0 + " while trying to fail enqueued calls.", new Exception());
                    break;
            ...
            case 19:
                ...
                break;
            default:
                int r10 = message.what;
                Log.w("GoogleApiManager", "Unknown message id: " + r10);
                return false;
        }
        return true;
    }
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public final boolean zaG(ConnectionResult connectionResult, int r4) {
        return this.zal.zah(this.zak, connectionResult, r4);
    }
    ...
    public final Task zar(GoogleApi googleApi, ListenerHolder.ListenerKey listenerKey, int r6) {
        TaskCompletionSource taskCompletionSource = new TaskCompletionSource();
        zaL(taskCompletionSource, r6, googleApi);
    ...
    public final void zaw(GoogleApi googleApi, int r4, BaseImplementation.ApiMethodImpl apiMethodImpl) {
        zae zaeVar = new zae(r4, apiMethodImpl);
        Handler handler = this.zat;
        handler.sendMessage(handler.obtainMessage(4, new zach(zaeVar, this.zao.get(), googleApi)));
    }

    public final void zax(GoogleApi googleApi, int r3, TaskApiCall taskApiCall, TaskCompletionSource taskCompletionSource, StatusExceptionMapper statusExceptionMapper) {
        zaL(taskCompletionSource, taskApiCall.zaa(), googleApi);
        zag zagVar = new zag(r3, taskApiCall, taskCompletionSource, statusExceptionMapper);
        Handler handler = this.zat;
        handler.sendMessage(handler.obtainMessage(4, new zach(zagVar, this.zao.get(), googleApi)));
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public final void zay(MethodInvocation methodInvocation, int r10, long j, int r13) {
        Handler handler = this.zat;
        handler.sendMessage(handler.obtainMessage(18, new zace(methodInvocation, r10, j, r13)));
    }
    ...
    public final void zaz(ConnectionResult connectionResult, int r5) {
        ...
        Handler handler = this.zat;
        handler.sendMessage(handler.obtainMessage(5, r5, 0, connectionResult));
    }
}
