...
public final class Tx3gDecoder extends SimpleSubtitleDecoder {
    ...
    public Tx3gDecoder(List<byte[]> list) {
        ...
        if (list.size() == 1 && (list.get(0).length == 48 || list.get(0).length == 53)) {
            ...
            this.defaultFontFamily = TX3G_SERIF.equals(Util.fromUtf8Bytes(bArr, 43, bArr.length - 43)) ? C1856C.SERIF_NAME : "sans-serif";
            int r0 = bArr[25] * Ascii.DC4;
            this.calculatedVideoTrackHeight = r0;
            boolean z = (bArr[0] & 32) != 0;
            this.customVerticalPlacement = z;
            if (z) {
                this.defaultVerticalPlacement = Util.constrainValue(((bArr[11] & 255) | ((bArr[10] & 255) << 8)) / r0, 0.0f, 0.95f);
                return;
            } else {
    ...
    protected Subtitle decode(byte[] bArr, int r8, boolean z) throws SubtitleDecoderException {
        this.parsableByteArray.reset(bArr, r8);
        ...
        while (this.parsableByteArray.bytesLeft() >= 8) {
            ...
            if (readInt2 == TYPE_STYL) {
                ...
                for (int r3 = 0; r3 < readUnsignedShort; r3++) {
    ...
    private void applyStyleRecord(ParsableByteArray parsableByteArray, SpannableStringBuilder spannableStringBuilder) throws SubtitleDecoderException {
        ...
        if (readUnsignedShort2 > spannableStringBuilder.length()) {
            Log.m1132w(TAG, "Truncating styl end (" + readUnsignedShort2 + ") to cueText.length() (" + spannableStringBuilder.length() + ").");
            r1 = spannableStringBuilder.length();
        } else {
            r1 = readUnsignedShort2;
        }
        if (readUnsignedShort >= r1) {
            Log.m1132w(TAG, "Ignoring styl with start (" + readUnsignedShort + ") >= end (" + r1 + ").");
            return;
        }
        int r6 = r1;
        attachFontFace(spannableStringBuilder, readUnsignedByte, this.defaultFontFace, readUnsignedShort, r6, 0);
        attachColor(spannableStringBuilder, readInt, this.defaultColorRgba, readUnsignedShort, r6, 0);
    }

    private static void attachFontFace(SpannableStringBuilder spannableStringBuilder, int r6, int r7, int r8, int r9, int r10) {
        if (r6 != r7) {
            int r72 = r10 | 33;
            boolean z = (r6 & 1) != 0;
            boolean z2 = (r6 & 2) != 0;
            if (z) {
                if (z2) {
                    spannableStringBuilder.setSpan(new StyleSpan(3), r8, r9, r72);
                } else {
                    spannableStringBuilder.setSpan(new StyleSpan(1), r8, r9, r72);
                }
            } else if (z2) {
                spannableStringBuilder.setSpan(new StyleSpan(2), r8, r9, r72);
            }
            boolean z3 = (r6 & 4) != 0;
            if (z3) {
                spannableStringBuilder.setSpan(new UnderlineSpan(), r8, r9, r72);
            }
            if (z3 || z || z2) {
                return;
            }
            spannableStringBuilder.setSpan(new StyleSpan(0), r8, r9, r72);
        }
    }

    private static void attachColor(SpannableStringBuilder spannableStringBuilder, int r1, int r2, int r3, int r4, int r5) {
        if (r1 != r2) {
            spannableStringBuilder.setSpan(new ForegroundColorSpan((r1 >>> 8) | ((r1 & 255) << 24)), r3, r4, r5 | 33);
        }
    }

    private static void attachFontFamily(SpannableStringBuilder spannableStringBuilder, String str, int r3, int r4) {
        if (str != "sans-serif") {
            spannableStringBuilder.setSpan(new TypefaceSpan(str), r3, r4, 16711713);
        }
    }
