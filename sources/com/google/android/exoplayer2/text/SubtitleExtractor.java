...
public class SubtitleExtractor implements Extractor {
    ...
    private int state = 0;
    private long seekTimeUs = C1856C.TIME_UNSET;
    ...
    public void init(ExtractorOutput extractorOutput) {
        ...
        this.extractorOutput.seekMap(new IndexSeekMap(new long[]{0}, new long[]{0}, C1856C.TIME_UNSET));
    ...
    public int read(ExtractorInput extractorInput, PositionHolder positionHolder) throws IOException {
        int r9 = this.state;
        Assertions.checkState((r9 == 0 || r9 == 5) ? false : true);
        if (this.state == 1) {
    ...
    public void seek(long j, long j2) {
        int r2 = this.state;
        Assertions.checkState((r2 == 0 || r2 == 5) ? false : true);
        this.seekTimeUs = j2;
        if (this.state == 2) {
            this.state = 1;
        }
    ...
    private boolean readFromInput(ExtractorInput extractorInput) throws IOException {
        ...
        int r1 = this.bytesRead;
        if (capacity == r1) {
            this.subtitleData.ensureCapacity(r1 + 1024);
        }
    ...
    private void decode() throws IOException {
        try {
            ...
            for (int r6 = 0; r6 < dequeueOutputBuffer.getEventTimeCount(); r6++) {
                byte[] encode = this.cueEncoder.encode(dequeueOutputBuffer.getCues(dequeueOutputBuffer.getEventTime(r6)));
                this.timestamps.add(Long.valueOf(dequeueOutputBuffer.getEventTime(r6)));
    ...
    private void writeToOutput() {
        ...
        for (int binarySearchFloor = j == C1856C.TIME_UNSET ? 0 : Util.binarySearchFloor((List<? extends Comparable<? super Long>>) this.timestamps, Long.valueOf(j), true, true); binarySearchFloor < this.samples.size(); binarySearchFloor++) {
