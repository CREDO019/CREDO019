...
public final class MediaMetricsListener implements AnalyticsListener, PlaybackSessionManager.Listener {
    ...
    private static int getTrackChangeReason(int r3) {
        if (r3 != 1) {
            if (r3 != 2) {
                return r3 != 3 ? 1 : 4;
            }
    ...
    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public /* synthetic */ void onDroppedVideoFrames(AnalyticsListener.EventTime eventTime, int r2, long j) {
        AnalyticsListener.CC.$default$onDroppedVideoFrames(this, eventTime, r2, j);
    }
    ...
    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public void onBandwidthEstimate(AnalyticsListener.EventTime eventTime, int r7, long j, long j2) {
        if (eventTime.mediaPeriodId != null) {
            ...
            this.bandwidthTimeMs.put(sessionForMediaPeriodId, Long.valueOf((l2 != null ? l2.longValue() : 0L) + r7));
        }
    }
    ...
    public void onDownstreamFormatChanged(AnalyticsListener.EventTime eventTime, MediaLoadData mediaLoadData) {
        ...
        int r6 = mediaLoadData.trackType;
        if (r6 != 0) {
            if (r6 == 1) {
    ...
    private void maybeUpdateVideoFormat(long j, Format format, int r10) {
        if (Util.areEqual(this.currentVideoFormat, format)) {
            return;
        }
        int r5 = (this.currentVideoFormat == null && r10 == 0) ? 1 : r10;
        this.currentVideoFormat = format;
        reportTrackChangeEvent(1, j, format, r5);
    }

    private void maybeUpdateAudioFormat(long j, Format format, int r10) {
        if (Util.areEqual(this.currentAudioFormat, format)) {
            return;
        }
        int r5 = (this.currentAudioFormat == null && r10 == 0) ? 1 : r10;
        this.currentAudioFormat = format;
        reportTrackChangeEvent(0, j, format, r5);
    }

    private void maybeUpdateTextFormat(long j, Format format, int r10) {
        if (Util.areEqual(this.currentTextFormat, format)) {
            return;
        }
        int r5 = (this.currentTextFormat == null && r10 == 0) ? 1 : r10;
        this.currentTextFormat = format;
        reportTrackChangeEvent(2, j, format, r5);
    }

    private void reportTrackChangeEvent(int r4, long j, Format format, int r8) {
        TrackChangeEvent.Builder timeSinceCreatedMillis = new TrackChangeEvent.Builder(r4).setTimeSinceCreatedMillis(j - this.startTimeMs);
        if (format != null) {
            timeSinceCreatedMillis.setTrackState(1);
            timeSinceCreatedMillis.setTrackChangeReason(getTrackChangeReason(r8));
    ...
    private void maybeUpdateTimelineMetadata(Timeline timeline, MediaSource.MediaPeriodId mediaPeriodId) {
        ...
        if (this.window.durationUs != C1856C.TIME_UNSET && !this.window.isPlaceholder && !this.window.isDynamic && !this.window.isLive()) {
    ...
    private static ErrorInfo getErrorInfo(PlaybackException playbackException, Context context, boolean z) {
        ...
        if (playbackException instanceof ExoPlaybackException) {
            ...
            r0 = exoPlaybackException.rendererFormatSupport;
        } else {
            r0 = 0;
            z2 = false;
        }
    ...
    private static DrmInitData getDrmInitData(ImmutableList<Tracks.Group> immutableList) {
        ...
        while (it.hasNext()) {
            ...
            for (int r1 = 0; r1 < next.length; r1++) {
                if (next.isTrackSelected(r1) && (drmInitData = next.getTrackFormat(r1).drmInitData) != null) {
                    return drmInitData;
                }
            }
        }
    ...
    private static int getDrmType(DrmInitData drmInitData) {
        for (int r0 = 0; r0 < drmInitData.schemeDataCount; r0++) {
            UUID r1 = drmInitData.get(r0).uuid;
            if (r1.equals(C1856C.WIDEVINE_UUID)) {
                return 3;
            }
            if (r1.equals(C1856C.PLAYREADY_UUID)) {
                return 2;
            }
            if (r1.equals(C1856C.CLEARKEY_UUID)) {
                return 6;
            }
        }
        return 1;
    ...
    private static int getDrmErrorCode(int r0) {
        switch (Util.getErrorCodeForMediaDrmErrorCode(r0)) {
    ...
    public static final class ErrorInfo {
        ...
        public ErrorInfo(int r1, int r2) {
            this.errorCode = r1;
            this.subErrorCode = r2;
        }
    }
    ...
    public static final class PendingFormatUpdate {
        ...
        public PendingFormatUpdate(Format format, int r2, String str) {
            this.format = format;
            this.selectionReason = r2;
            this.sessionId = str;
        }
    }
}
