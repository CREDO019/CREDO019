...
public final class DefaultPlaybackSessionManager implements PlaybackSessionManager {
    ...
    */
    ...
    @Override // com.google.android.exoplayer2.analytics.PlaybackSessionManager
    public synchronized void updateSessionsWithDiscontinuity(AnalyticsListener.EventTime eventTime, int r8) {
        Assertions.checkNotNull(this.listener);
        boolean z = r8 == 0;
        Iterator<SessionDescriptor> it = this.sessions.values().iterator();
        while (it.hasNext()) {
    ...
    private SessionDescriptor getOrAddSession(int r11, MediaSource.MediaPeriodId mediaPeriodId) {
        int r7;
        SessionDescriptor sessionDescriptor = null;
        long j = Long.MAX_VALUE;
        for (SessionDescriptor sessionDescriptor2 : this.sessions.values()) {
            sessionDescriptor2.maybeSetWindowSequenceNumber(r11, mediaPeriodId);
            if (sessionDescriptor2.belongsToSession(r11, mediaPeriodId)) {
                long j2 = sessionDescriptor2.windowSequenceNumber;
                if (j2 == -1 || j2 < j) {
                    sessionDescriptor = sessionDescriptor2;
                    j = j2;
                } else if (r7 == 0 && ((SessionDescriptor) Util.castNonNull(sessionDescriptor)).adMediaPeriodId != null && sessionDescriptor2.adMediaPeriodId != null) {
                    sessionDescriptor = sessionDescriptor2;
                }
            }
        }
        if (sessionDescriptor == null) {
            ...
            SessionDescriptor sessionDescriptor3 = new SessionDescriptor(str, r11, mediaPeriodId);
    ...
    public final class SessionDescriptor {
        ...
        public SessionDescriptor(String str, int r3, MediaSource.MediaPeriodId mediaPeriodId) {
            this.sessionId = str;
            this.windowIndex = r3;
        ...
        public boolean belongsToSession(int r7, MediaSource.MediaPeriodId mediaPeriodId) {
            return mediaPeriodId == null ? r7 == this.windowIndex : this.adMediaPeriodId == null ? !mediaPeriodId.isAd() && mediaPeriodId.windowSequenceNumber == this.windowSequenceNumber : mediaPeriodId.windowSequenceNumber == this.adMediaPeriodId.windowSequenceNumber && mediaPeriodId.adGroupIndex == this.adMediaPeriodId.adGroupIndex && mediaPeriodId.adIndexInAdGroup == this.adMediaPeriodId.adIndexInAdGroup;
        }

        public void maybeSetWindowSequenceNumber(int r6, MediaSource.MediaPeriodId mediaPeriodId) {
            if (this.windowSequenceNumber == -1 && r6 == this.windowIndex && mediaPeriodId != null) {
        ...
        public boolean isFinishedAtEventTime(AnalyticsListener.EventTime eventTime) {
            ...
            if (eventTime.mediaPeriodId == null) {
                ...
                int r0 = eventTime.mediaPeriodId.adGroupIndex;
                return r0 > this.adMediaPeriodId.adGroupIndex || (r0 == this.adMediaPeriodId.adGroupIndex && eventTime.mediaPeriodId.adIndexInAdGroup > this.adMediaPeriodId.adIndexInAdGroup);
            }
        }

        private int resolveWindowIndexToNewTimeline(Timeline timeline, Timeline timeline2, int r5) {
            if (r5 < timeline.getWindowCount()) {
                timeline.getWindow(r5, DefaultPlaybackSessionManager.this.window);
                for (int r52 = DefaultPlaybackSessionManager.this.window.firstPeriodIndex; r52 <= DefaultPlaybackSessionManager.this.window.lastPeriodIndex; r52++) {
                    int indexOfPeriod = timeline2.getIndexOfPeriod(timeline.getUidOfPeriod(r52));
                    if (indexOfPeriod != -1) {
                        return timeline2.getPeriod(indexOfPeriod, DefaultPlaybackSessionManager.this.period).windowIndex;
                    }
                }
                return -1;
            } else if (r5 < timeline2.getWindowCount()) {
                return r5;
            } else {
                return -1;
            }
