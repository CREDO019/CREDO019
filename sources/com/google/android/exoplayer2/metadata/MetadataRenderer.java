...
public final class MetadataRenderer extends BaseRenderer implements Handler.Callback {
    ...
    public MetadataRenderer(MetadataOutput metadataOutput, Looper looper, MetadataDecoderFactory metadataDecoderFactory) {
        ...
        this.pendingMetadataTimestampUs = C1856C.TIME_UNSET;
    }
    ...
    private void decodeWrappedMetadata(Metadata metadata, List<Metadata.Entry> list) {
        for (int r0 = 0; r0 < metadata.length(); r0++) {
            Format wrappedMetadataFormat = metadata.get(r0).getWrappedMetadataFormat();
            if (wrappedMetadataFormat != null && this.decoderFactory.supportsFormat(wrappedMetadataFormat)) {
                MetadataDecoder createDecoder = this.decoderFactory.createDecoder(wrappedMetadataFormat);
                byte[] bArr = (byte[]) Assertions.checkNotNull(metadata.get(r0).getWrappedMetadataBytes());
                this.buffer.clear();
