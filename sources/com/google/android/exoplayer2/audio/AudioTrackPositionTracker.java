...
final class AudioTrackPositionTracker {
    ...
    public void setAudioTrack(AudioTrack audioTrack, boolean z, int r5, int r6, int r7) {
        this.audioTrack = audioTrack;
        this.outputPcmFrameSize = r6;
        this.bufferSize = r7;
        this.audioTimestampPoller = new AudioTimestampPoller(audioTrack);
        this.outputSampleRate = audioTrack.getSampleRate();
        this.needsPassthroughWorkarounds = z && needsPassthroughWorkarounds(r5);
        boolean isEncodingLinearPcm = Util.isEncodingLinearPcm(r5);
        this.isOutputPcm = isEncodingLinearPcm;
        this.bufferSizeUs = isEncodingLinearPcm ? framesToDurationUs(r7 / r6) : -9223372036854775807L;
        this.lastRawPlaybackHeadPosition = 0L;
        this.rawPlaybackHeadWrapCount = 0L;
        this.passthroughWorkaroundPauseOffset = 0L;
        this.hasData = false;
        this.stopTimestampUs = C1856C.TIME_UNSET;
        this.forceResetWorkaroundTimeMs = C1856C.TIME_UNSET;
    ...
    public boolean isStalled(long j) {
        return this.forceResetWorkaroundTimeMs != C1856C.TIME_UNSET && j > 0 && SystemClock.elapsedRealtime() - this.forceResetWorkaroundTimeMs >= FORCE_RESET_WORKAROUND_TIMEOUT_MS;
    }
    ...
    public boolean pause() {
        resetSyncParams();
        if (this.stopTimestampUs == C1856C.TIME_UNSET) {
            ((AudioTimestampPoller) Assertions.checkNotNull(this.audioTimestampPoller)).reset();
            return true;
        }
        return false;
    ...
    private void maybeSampleSyncParams() {
        ...
        if (nanoTime - this.lastPlayheadSampleTimeUs >= 30000) {
            ...
            int r7 = this.nextPlayheadOffsetIndex;
            jArr[r7] = playbackHeadPositionUs - nanoTime;
            this.nextPlayheadOffsetIndex = (r7 + 1) % 10;
            int r72 = this.playheadOffsetCount;
            if (r72 < 10) {
                this.playheadOffsetCount = r72 + 1;
            }
            this.lastPlayheadSampleTimeUs = nanoTime;
            this.smoothedPlayheadOffsetUs = 0L;
            int r2 = 0;
            while (true) {
                int r3 = this.playheadOffsetCount;
                if (r2 >= r3) {
                    break;
                }
                this.smoothedPlayheadOffsetUs += this.playheadOffsets[r2] / r3;
                r2++;
            }
        }
        if (this.needsPassthroughWorkarounds) {
    ...
    private static boolean needsPassthroughWorkarounds(int r2) {
        return Util.SDK_INT < 23 && (r2 == 5 || r2 == 6);
    }
    ...
    private long getPlaybackHeadPosition() {
        ...
        if (Util.SDK_INT <= 29) {
            if (playbackHeadPosition == 0 && this.lastRawPlaybackHeadPosition > 0 && playState == 3) {
                if (this.forceResetWorkaroundTimeMs == C1856C.TIME_UNSET) {
