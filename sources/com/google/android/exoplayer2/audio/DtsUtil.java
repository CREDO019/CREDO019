...
public final class DtsUtil {
    ...
    private static final int[] TWICE_BITRATE_KBPS_BY_RATE = {64, 112, 128, 192, 224, 256, BitmapCounterConfig.DEFAULT_MAX_BITMAP_COUNT, 448, 512, 640, 768, 896, 1024, 1152, 1280, 1536, 1920, 2048, 2304, 2560, 2688, 2816, 2823, 2944, 3072, 3840, 4096, 6144, 7680};

    public static boolean isSyncWord(int r1) {
        return r1 == SYNC_VALUE_BE || r1 == SYNC_VALUE_LE || r1 == SYNC_VALUE_14B_BE || r1 == SYNC_VALUE_14B_LE;
    }
    ...
    public static Format parseDtsFormat(byte[] bArr, String str, String str2, DrmInitData drmInitData) {
        ...
        int r0 = CHANNELS_BY_AMODE[normalizedFrameHeader.readBits(6)];
        int r1 = SAMPLE_RATE_BY_SFREQ[normalizedFrameHeader.readBits(4)];
        int readBits = normalizedFrameHeader.readBits(5);
        int[] r3 = TWICE_BITRATE_KBPS_BY_RATE;
        int r2 = readBits >= r3.length ? -1 : (r3[readBits] * 1000) / 2;
        normalizedFrameHeader.skipBits(10);
        return new Format.Builder().setId(str).setSampleMimeType(MimeTypes.AUDIO_DTS).setAverageBitrate(r2).setChannelCount(r0 + (normalizedFrameHeader.readBits(2) > 0 ? 1 : 0)).setSampleRate(r1).setDrmInitData(drmInitData).setLanguage(str2).build();
    }
    ...
    public static int parseDtsAudioSampleCount(ByteBuffer byteBuffer) {
        ...
        if (b3 != -2) {
            if (b3 == -1) {
                r1 = (byteBuffer.get(position + 4) & 7) << 4;
                b2 = byteBuffer.get(position + 7);
            } else if (b3 == 31) {
                r1 = (byteBuffer.get(position + 5) & 7) << 4;
                b2 = byteBuffer.get(position + 6);
            } else {
                r1 = (byteBuffer.get(position + 4) & 1) << 6;
                b = byteBuffer.get(position + 5);
            }
            r3 = b2 & 60;
            return (((r3 >> 2) | r1) + 1) * 32;
        }
        r1 = (byteBuffer.get(position + 5) & 1) << 6;
    ...
    */
    ...
    private static ParsableBitArray getNormalizedFrameHeader(byte[] bArr) {
        ...
        if (isLittleEndianFrameHeader(copyOf)) {
            for (int r1 = 0; r1 < copyOf.length - 1; r1 += 2) {
                byte b = copyOf[r1];
                int r3 = r1 + 1;
                copyOf[r1] = copyOf[r3];
                copyOf[r3] = b;
