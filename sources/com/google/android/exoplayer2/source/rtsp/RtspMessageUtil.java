...
public final class RtspMessageUtil {
    ...
    /* renamed from: LF */
    private static final String f245LF = new String(new byte[]{10});
    ...
    public static ImmutableList<String> serializeRequest(RtspRequest rtspRequest) {
        ...
        while (it.hasNext()) {
            ...
            for (int r8 = 0; r8 < immutableList.size(); r8++) {
                builder.add((ImmutableList.Builder) Util.formatInvariant("%s: %s", next, immutableList.get(r8)));
            }
        }
    ...
    public static byte[] convertMessageToByteArray(List<String> list) {
        return Joiner.m440on(CRLF).join(list).getBytes(RtspMessageChannel.CHARSET);
    }
    ...
    public static RtspAuthUserInfo parseUserInfo(Uri uri) {
        ...
        if (userInfo != null && userInfo.contains(ParameterizedMessage.ERROR_MSG_SEPARATOR)) {
            String[] splitAtFirst = Util.splitAtFirst(userInfo, ParameterizedMessage.ERROR_MSG_SEPARATOR);
    ...
    public static String toMethodString(int r0) {
        switch (r0) {
            case 1:
                return "ANNOUNCE";
            case 2:
    ...
    public static RtspResponse parseResponse(List<String> list) {
        ...
        return new RtspResponse(parseInt, new RtspHeaders.Builder().addAll(list.subList(1, indexOf)).build(), Joiner.m440on(CRLF).join(list.subList(indexOf + 1, list.size())));
    }
    ...
    public static RtspRequest parseRequest(List<String> list) {
        ...
        int indexOf = list.indexOf("");
        Assertions.checkArgument(indexOf > 0);
        return new RtspRequest(parse, parseMethodString, new RtspHeaders.Builder().addAll(list.subList(1, indexOf)).build(), Joiner.m440on(CRLF).join(list.subList(indexOf + 1, list.size())));
    }
    ...
    private static String getRtspStatusReasonPhrase(int r1) {
        if (r1 != 200) {
            if (r1 != 461) {
                if (r1 != 500) {
                    if (r1 != 505) {
                        if (r1 != 301) {
                            if (r1 != 302) {
                                if (r1 != 400) {
                                    ...
                                    return Constants.BluetoothState.UNAUTHORIZED;
                                }
                                return "Bad Request";
                            }
