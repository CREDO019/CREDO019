...
public final class RtpPacket {
    ...
    public static final class Builder {
        ...
        public Builder setSequenceNumber(int r3) {
            Assertions.checkArgument(r3 >= 0 && r3 <= 65535);
            this.sequenceNumber = r3 & 65535;
            return this;
        }
    ...
    public static int getNextSequenceNumber(int r1) {
        return IntMath.mod(r1 + 1, 65536);
    }

    public static int getPreviousSequenceNumber(int r1) {
        return IntMath.mod(r1 - 1, 65536);
    }
    ...
    public static RtpPacket parse(ParsableByteArray parsableByteArray) {
        ...
        if (b2 > 0) {
            bArr = new byte[b2 * 4];
            for (int r10 = 0; r10 < b2; r10++) {
                parsableByteArray.readBytes(bArr, r10 * 4, 4);
            }
        } else {
            bArr = EMPTY;
        }
    ...
    public static RtpPacket parse(byte[] bArr, int r2) {
        return parse(new ParsableByteArray(bArr, r2));
    }
    ...
    public int writeToBuffer(byte[] bArr, int r4, int r5) {
        int length = (this.csrcCount * 4) + 12 + this.payloadData.length;
        if (r5 < length || bArr.length - r4 < length) {
            return -1;
        }
        ByteBuffer.wrap(bArr, r4, r5).put((byte) (((this.padding ? 1 : 0) << 5) | 128 | ((this.extension ? 1 : 0) << 4) | (this.csrcCount & Ascii.f1128SI))).put((byte) (((this.marker ? 1 : 0) << 7) | (this.payloadType & Byte.MAX_VALUE))).putShort((short) this.sequenceNumber).putInt((int) this.timestamp).putInt(this.ssrc).put(this.csrc).put(this.payloadData);
        return length;
    }
