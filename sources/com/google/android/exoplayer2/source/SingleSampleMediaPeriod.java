...
public final class SingleSampleMediaPeriod implements MediaPeriod, Loader.Callback<SourceLoadable> {
    ...
    public long selectTracks(ExoTrackSelection[] exoTrackSelectionArr, boolean[] zArr, SampleStream[] sampleStreamArr, boolean[] zArr2, long j) {
        for (int r0 = 0; r0 < exoTrackSelectionArr.length; r0++) {
            if (sampleStreamArr[r0] != null && (exoTrackSelectionArr[r0] == null || !zArr[r0])) {
                this.sampleStreams.remove(sampleStreamArr[r0]);
                sampleStreamArr[r0] = null;
            }
            if (sampleStreamArr[r0] == null && exoTrackSelectionArr[r0] != null) {
                SampleStreamImpl sampleStreamImpl = new SampleStreamImpl();
                this.sampleStreams.add(sampleStreamImpl);
                sampleStreamArr[r0] = sampleStreamImpl;
                zArr2[r0] = true;
            }
        }
        return j;
    ...
    public long seekToUs(long j) {
        for (int r0 = 0; r0 < this.sampleStreams.size(); r0++) {
            this.sampleStreams.get(r0).reset();
        }
        return j;
    }
    ...
    @Override // com.google.android.exoplayer2.upstream.Loader.Callback
    public Loader.LoadErrorAction onLoadError(SourceLoadable sourceLoadable, long j, long j2, IOException iOException, int r43) {
        Loader.LoadErrorAction loadErrorAction;
        StatsDataSource statsDataSource = sourceLoadable.dataSource;
        LoadEventInfo loadEventInfo = new LoadEventInfo(sourceLoadable.loadTaskId, sourceLoadable.dataSpec, statsDataSource.getLastOpenedUri(), statsDataSource.getLastResponseHeaders(), j, j2, statsDataSource.getBytesRead());
        long retryDelayMsFor = this.loadErrorHandlingPolicy.getRetryDelayMsFor(new LoadErrorHandlingPolicy.LoadErrorInfo(loadEventInfo, new MediaLoadData(1, -1, this.format, 0, null, 0L, Util.usToMs(this.durationUs)), iOException, r43));
        int r10 = (retryDelayMsFor > C1856C.TIME_UNSET ? 1 : (retryDelayMsFor == C1856C.TIME_UNSET ? 0 : -1));
        boolean z = r10 == 0 || r43 >= this.loadErrorHandlingPolicy.getMinimumLoadableRetryCount(1);
        if (this.treatLoadErrorsAsEndOfStream && z) {
            Log.m1131w(TAG, "Loading failed, treating as end-of-stream.", iOException);
            this.loadingFinished = true;
    ...
    private final class SampleStreamImpl implements SampleStream {
        ...
        public int readData(FormatHolder formatHolder, DecoderInputBuffer decoderInputBuffer, int r10) {
            ...
            int r0 = this.streamState;
            if (r0 == 2) {
                decoderInputBuffer.addFlag(4);
                return -4;
            } else if ((r10 & 2) != 0 || r0 == 0) {
        ...
        public void load() throws IOException {
            ...
            try {
                ...
                while (r0 != -1) {
                    ...
                    r0 = statsDataSource.read(bArr2, bytesRead, bArr2.length - bytesRead);
                }
            } finally {
