...
public class DefaultDashChunkSource implements DashChunkSource {
    ...
    public static final class Factory implements DashChunkSource.Factory {
        ...
        public Factory(DataSource.Factory factory, int r3) {
            this(BundledChunkExtractor.FACTORY, factory, r3);
        }

        public Factory(ChunkExtractor.Factory factory, DataSource.Factory factory2, int r3) {
            this.chunkExtractorFactory = factory;
            this.dataSourceFactory = factory2;
            this.maxSegmentsPerLoad = r3;
        }

        @Override // com.google.android.exoplayer2.source.dash.DashChunkSource.Factory
        public DashChunkSource createDashChunkSource(LoaderErrorThrower loaderErrorThrower, DashManifest dashManifest, BaseUrlExclusionList baseUrlExclusionList, int r24, int[] r25, ExoTrackSelection exoTrackSelection, int r27, long j, boolean z, List<Format> list, PlayerEmsgHandler.PlayerTrackEmsgHandler playerTrackEmsgHandler, TransferListener transferListener, PlayerId playerId) {
            DataSource createDataSource = this.dataSourceFactory.createDataSource();
            if (transferListener != null) {
                createDataSource.addTransferListener(transferListener);
            }
            return new DefaultDashChunkSource(this.chunkExtractorFactory, loaderErrorThrower, dashManifest, baseUrlExclusionList, r24, r25, exoTrackSelection, r27, createDataSource, j, this.maxSegmentsPerLoad, z, list, playerTrackEmsgHandler, playerId);
        }
    }

    public DefaultDashChunkSource(ChunkExtractor.Factory factory, LoaderErrorThrower loaderErrorThrower, DashManifest dashManifest, BaseUrlExclusionList baseUrlExclusionList, int r32, int[] r33, ExoTrackSelection exoTrackSelection, int r35, DataSource dataSource, long j, int r39, boolean z, List<Format> list, PlayerEmsgHandler.PlayerTrackEmsgHandler playerTrackEmsgHandler, PlayerId playerId) {
        this.manifestLoaderErrorThrower = loaderErrorThrower;
        this.manifest = dashManifest;
        this.baseUrlExclusionList = baseUrlExclusionList;
        this.adaptationSetIndices = r33;
        this.trackSelection = exoTrackSelection;
        this.trackType = r35;
        this.dataSource = dataSource;
        this.periodIndex = r32;
        this.elapsedRealtimeOffsetMs = j;
        this.maxSegmentsPerLoad = r39;
        this.playerTrackEmsgHandler = playerTrackEmsgHandler;
        long periodDurationUs = dashManifest.getPeriodDurationUs(r32);
        ArrayList<Representation> representations = getRepresentations();
        this.representationHolders = new RepresentationHolder[exoTrackSelection.length()];
        int r15 = 0;
        while (r15 < this.representationHolders.length) {
            Representation representation = representations.get(exoTrackSelection.getIndexInTrackGroup(r15));
            ...
            int r6 = r15;
            representationHolderArr[r6] = new RepresentationHolder(periodDurationUs, representation, selectBaseUrl, factory.createProgressiveMediaExtractor(r35, representation.format, z, list, playerTrackEmsgHandler, playerId), 0L, representation.getIndex());
            r15 = r6 + 1;
        }
    }
    ...
    public void updateManifest(DashManifest dashManifest, int r7) {
        try {
            this.manifest = dashManifest;
            this.periodIndex = r7;
            long periodDurationUs = dashManifest.getPeriodDurationUs(r7);
            ArrayList<Representation> representations = getRepresentations();
            for (int r1 = 0; r1 < this.representationHolders.length; r1++) {
                RepresentationHolder[] representationHolderArr = this.representationHolders;
                representationHolderArr[r1] = representationHolderArr[r1].copyWithNewRepresentation(periodDurationUs, representations.get(this.trackSelection.getIndexInTrackGroup(r1)));
            }
    ...
    public void getNextChunk(long j, long j2, List<? extends MediaChunk> list, ChunkHolder chunkHolder) {
        ...
        if (playerTrackEmsgHandler == null || !playerTrackEmsgHandler.maybeRefreshManifestBeforeLoadingNextChunk(msToUs)) {
            ...
            int r2 = 0;
            while (r2 < length) {
                RepresentationHolder representationHolder = this.representationHolders[r2];
                if (representationHolder.segmentIndex == null) {
                    mediaChunkIteratorArr2[r2] = MediaChunkIterator.EMPTY;
                    r13 = r2;
                    r28 = length;
                    ...
                    if (segmentNum < firstAvailableSegmentNum) {
                        mediaChunkIteratorArr[r13] = MediaChunkIterator.EMPTY;
                    } else {
                        mediaChunkIteratorArr[r13] = new RepresentationSegmentIterator(updateSelectedBaseUrl(r13), segmentNum, lastAvailableSegmentNum, nowPeriodTimeUs);
                    }
                }
                r2 = r13 + 1;
                msToUs2 = j4;
            ...
            long j9 = C1856C.TIME_UNSET;
            int r10 = (j8 > C1856C.TIME_UNSET ? 1 : (j8 == C1856C.TIME_UNSET ? 0 : -1));
            boolean z = r10 != 0;
            ...
            int r0 = (segmentNum2 > lastAvailableSegmentNum2 ? 1 : (segmentNum2 == lastAvailableSegmentNum2 ? 0 : -1));
            if (r0 > 0 || (this.missingLastSegment && r0 >= 0)) {
                chunkHolder.endOfStream = z;
                ...
                chunkHolder.chunk = newMediaChunk(updateSelectedBaseUrl, this.dataSource, this.trackType, this.trackSelection.getSelectedFormat(), this.trackSelection.getSelectionReason(), this.trackSelection.getSelectionData(), segmentNum2, r102, j9, nowPeriodTimeUs);
            }
        }
    }
    ...
    private LoadErrorHandlingPolicy.FallbackOptions createFallbackOptions(ExoTrackSelection exoTrackSelection, List<BaseUrl> list) {
        ...
        for (int r3 = 0; r3 < length; r3++) {
            if (exoTrackSelection.isBlacklisted(r3, elapsedRealtime)) {
                r4++;
            }
        }
        int priorityCount = BaseUrlExclusionList.getPriorityCount(list);
        return new LoadErrorHandlingPolicy.FallbackOptions(priorityCount, priorityCount - this.baseUrlExclusionList.getPriorityCountAfterExclusion(list), length, r4);
    }
    ...
    private ArrayList<Representation> getRepresentations() {
        ...
        for (int r5 : this.adaptationSetIndices) {
            arrayList.addAll(list.get(r5).representations);
        }
        return arrayList;
    }
    ...
    private long getNowPeriodTimeUs(long j) {
        return this.manifest.availabilityStartTimeMs == C1856C.TIME_UNSET ? C1856C.TIME_UNSET : j - Util.msToUs(this.manifest.availabilityStartTimeMs + this.manifest.getPeriod(this.periodIndex).startMs);
    }

    protected Chunk newInitializationChunk(RepresentationHolder representationHolder, DataSource dataSource, Format format, int r16, Object obj, RangedUri rangedUri, RangedUri rangedUri2) {
        ...
        return new InitializationChunk(dataSource, DashUtil.buildDataSpec(representation, representationHolder.selectedBaseUrl.url, rangedUri3, 0), format, r16, obj, representationHolder.chunkExtractor);
    }

    protected Chunk newMediaChunk(RepresentationHolder representationHolder, DataSource dataSource, int r27, Format format, int r29, Object obj, long j, int r33, long j2, long j3) {
        Representation representation = representationHolder.representation;
        ...
        if (representationHolder.chunkExtractor == null) {
            return new SingleSampleMediaChunk(dataSource, DashUtil.buildDataSpec(representation, representationHolder.selectedBaseUrl.url, segmentUrl, representationHolder.isSegmentAvailableAtFullNetworkSpeed(j, j3) ? 0 : 8), format, r29, obj, segmentStartTimeUs, representationHolder.getSegmentEndTimeUs(j), j, r27, format);
        }
        int r5 = 1;
        int r15 = 1;
        while (r5 < r33) {
            RangedUri attemptMerge = segmentUrl.attemptMerge(representationHolder.getSegmentUrl(r5 + j), representationHolder.selectedBaseUrl.url);
            if (attemptMerge == null) {
                break;
            }
            r15++;
        ...
        return new ContainerMediaChunk(dataSource, DashUtil.buildDataSpec(representation, representationHolder.selectedBaseUrl.url, segmentUrl, representationHolder.isSegmentAvailableAtFullNetworkSpeed(j4, j3) ? 0 : 8), format, r29, obj, segmentStartTimeUs, segmentEndTimeUs, j2, (j5 == C1856C.TIME_UNSET || j5 > segmentEndTimeUs) ? -9223372036854775807L : j5, j, r15, -representation.presentationTimeOffsetUs, representationHolder.chunkExtractor);
    }

    private RepresentationHolder updateSelectedBaseUrl(int r4) {
        RepresentationHolder representationHolder = this.representationHolders[r4];
        ...
        this.representationHolders[r4] = copyWithNewSelectedBaseUrl;
    ...
    public static final class RepresentationHolder {
        ...
        RepresentationHolder copyWithNewRepresentation(long j, Representation representation) throws BehindLiveWindowException {
            ...
            int r21 = ((index.getTimeUs(j2) + index.getDurationUs(j2, j)) > timeUs2 ? 1 : ((index.getTimeUs(j2) + index.getDurationUs(j2, j)) == timeUs2 ? 0 : -1));
            if (r21 == 0) {
                segmentNum = j2 + 1;
            } else if (r21 < 0) {
        ...
        public boolean isSegmentAvailableAtFullNetworkSpeed(long j, long j2) {
            return this.segmentIndex.isExplicit() || j2 == C1856C.TIME_UNSET || getSegmentEndTimeUs(j) <= j2;
        }
    }
}
