...
public final class GlUtil {
    ...
    public static float[] createVertexBuffer(List<float[]> list) {
        ...
        for (int r3 = 0; r3 < list.size(); r3++) {
            System.arraycopy(list.get(r3), 0, fArr, r3 * 4, 4);
        }
        return fArr;
    }
    ...
    private static EGLSurface createPbufferSurface(EGLDisplay eGLDisplay, int r4, int r5) {
        return Api17.createEglPbufferSurface(eGLDisplay, EGL_CONFIG_ATTRIBUTES_RGBA_8888, new int[]{12375, r4, 12374, r5, 12344});
    }
    ...
    public static void checkGlError() {
        ...
        while (true) {
            ...
            Log.m1136e(TAG, "glError: " + GLU.gluErrorString(glGetError));
            r0 = glGetError;
        }
        if (r0 != 0) {
            throwGlException("glError: " + GLU.gluErrorString(r0));
        }
    }

    public static void assertValidTextureSize(int r3, int r4) {
        int[] r0 = new int[1];
        GLES20.glGetIntegerv(3379, r0, 0);
        int r02 = r0[0];
        if (r3 < 0 || r4 < 0) {
            throwGlException("width or height is less than 0");
        }
        if (r3 > r02 || r4 > r02) {
            throwGlException("width or height is greater than GL_MAX_TEXTURE_SIZE " + r02);
        }
    }

    public static void focusEglSurface(EGLDisplay eGLDisplay, EGLContext eGLContext, EGLSurface eGLSurface, int r9, int r10) {
        Api17.focusRenderTarget(eGLDisplay, eGLContext, eGLSurface, 0, r9, r10);
    }

    public static void focusFramebuffer(EGLDisplay eGLDisplay, EGLContext eGLContext, EGLSurface eGLSurface, int r3, int r4, int r5) {
        Api17.focusRenderTarget(eGLDisplay, eGLContext, eGLSurface, r3, r4, r5);
    }

    public static void deleteTexture(int r3) {
        GLES20.glDeleteTextures(1, new int[]{r3}, 0);
        checkGlError();
    }
    ...
    public static FloatBuffer createBuffer(int r1) {
        return ByteBuffer.allocateDirect(r1 * 4).order(ByteOrder.nativeOrder()).asFloatBuffer();
    }
    ...
    public static int createTexture(int r11, int r12) {
        assertValidTextureSize(r11, r12);
        int generateTexture = generateTexture();
        bindTexture(3553, generateTexture);
        GLES20.glTexImage2D(3553, 0, 6408, r11, r12, 0, 6408, 5121, ByteBuffer.allocateDirect(r11 * r12 * 4));
        checkGlError();
        return generateTexture;
    }
    ...
    private static int generateTexture() {
        ...
        int[] r0 = new int[1];
        GLES20.glGenTextures(1, r0, 0);
        checkGlError();
        return r0[0];
    }

    public static void bindTexture(int r1, int r2) {
    ...
    public static int createFboForTexture(int r5) {
        checkEglException(!Util.areEqual(EGL14.eglGetCurrentContext(), EGL14.EGL_NO_CONTEXT), "No current context");
        int[] r0 = new int[1];
        GLES20.glGenFramebuffers(1, r0, 0);
        checkGlError();
        GLES20.glBindFramebuffer(36160, r0[0]);
        checkGlError();
        GLES20.glFramebufferTexture2D(36160, 36064, 3553, r5, 0);
        checkGlError();
        return r0[0];
    }
    ...
    public static final class Api17 {
        ...
        public static EGLContext createEglContext(EGLDisplay eGLDisplay, int r5, int[] r6) {
            EGLContext eglCreateContext = EGL14.eglCreateContext(eGLDisplay, getEglConfig(eGLDisplay, r6), EGL14.EGL_NO_CONTEXT, new int[]{12440, r5, 12344}, 0);
            if (eglCreateContext == null) {
                EGL14.eglTerminate(eGLDisplay);
                GlUtil.throwGlException("eglCreateContext() failed to create a valid context. The device may not support EGL version " + r5);
            }
            GlUtil.checkGlError();
            return eglCreateContext;
        }

        public static EGLSurface getEglSurface(EGLDisplay eGLDisplay, Object obj, int[] r3, int[] r4) {
            EGLSurface eglCreateWindowSurface = EGL14.eglCreateWindowSurface(eGLDisplay, getEglConfig(eGLDisplay, r3), obj, r4, 0);
            GlUtil.checkEglException("Error creating surface");
            return eglCreateWindowSurface;
        }

        public static EGLSurface createEglPbufferSurface(EGLDisplay eGLDisplay, int[] r2, int[] r3) {
            EGLSurface eglCreatePbufferSurface = EGL14.eglCreatePbufferSurface(eGLDisplay, getEglConfig(eGLDisplay, r2), r3, 0);
            GlUtil.checkEglException("Error creating surface");
            return eglCreatePbufferSurface;
        }

        public static void focusRenderTarget(EGLDisplay eGLDisplay, EGLContext eGLContext, EGLSurface eGLSurface, int r6, int r7, int r8) {
            int[] r0 = new int[1];
            GLES20.glGetIntegerv(36006, r0, 0);
            if (r0[0] != r6) {
        ...
        private static EGLConfig getEglConfig(EGLDisplay eGLDisplay, int[] r11) {
            EGLConfig[] eGLConfigArr = new EGLConfig[1];
            if (!EGL14.eglChooseConfig(eGLDisplay, r11, 0, eGLConfigArr, 0, 1, new int[1], 0)) {
