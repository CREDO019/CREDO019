...
public class EventLogger implements AnalyticsListener {
    ...
    private static String getDiscontinuityReasonString(int r1) {
        return r1 != 0 ? r1 != 1 ? r1 != 2 ? r1 != 3 ? r1 != 4 ? r1 != 5 ? "?" : "INTERNAL" : "REMOVE" : "SKIP" : "SEEK_ADJUSTMENT" : "SEEK" : "AUTO_TRANSITION";
    }

    private static String getMediaItemTransitionReasonString(int r1) {
        return r1 != 0 ? r1 != 1 ? r1 != 2 ? r1 != 3 ? "?" : "PLAYLIST_CHANGED" : "SEEK" : "AUTO" : "REPEAT";
    }

    private static String getPlayWhenReadyChangeReasonString(int r1) {
        return r1 != 1 ? r1 != 2 ? r1 != 3 ? r1 != 4 ? r1 != 5 ? "?" : "END_OF_MEDIA_ITEM" : "REMOTE" : "AUDIO_BECOMING_NOISY" : "AUDIO_FOCUS_LOSS" : "USER_REQUEST";
    }

    private static String getPlaybackSuppressionReasonString(int r1) {
        return r1 != 0 ? r1 != 1 ? "?" : "TRANSIENT_AUDIO_FOCUS_LOSS" : "NONE";
    }

    private static String getRepeatModeString(int r1) {
        return r1 != 0 ? r1 != 1 ? r1 != 2 ? "?" : Rule.ALL : "ONE" : "OFF";
    }

    private static String getStateString(int r1) {
        return r1 != 1 ? r1 != 2 ? r1 != 3 ? r1 != 4 ? "?" : "ENDED" : "READY" : "BUFFERING" : "IDLE";
    }

    private static String getTimelineChangeReasonString(int r1) {
        return r1 != 0 ? r1 != 1 ? "?" : "SOURCE_UPDATE" : "PLAYLIST_CHANGED";
    }
    ...
    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public void onPlaybackStateChanged(AnalyticsListener.EventTime eventTime, int r3) {
        logd(eventTime, "state", getStateString(r3));
    }

    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public void onPlayWhenReadyChanged(AnalyticsListener.EventTime eventTime, boolean z, int r4) {
        logd(eventTime, "playWhenReady", z + ", " + getPlayWhenReadyChangeReasonString(r4));
    }

    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public void onPlaybackSuppressionReasonChanged(AnalyticsListener.EventTime eventTime, int r3) {
        logd(eventTime, "playbackSuppressionReason", getPlaybackSuppressionReasonString(r3));
    }
    ...
    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public void onRepeatModeChanged(AnalyticsListener.EventTime eventTime, int r3) {
        logd(eventTime, "repeatMode", getRepeatModeString(r3));
    }
    ...
    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public void onPositionDiscontinuity(AnalyticsListener.EventTime eventTime, Player.PositionInfo positionInfo, Player.PositionInfo positionInfo2, int r14) {
        StringBuilder sb = new StringBuilder();
        sb.append("reason=");
        sb.append(getDiscontinuityReasonString(r14));
    ...
    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public void onTimelineChanged(AnalyticsListener.EventTime eventTime, int r10) {
        int periodCount = eventTime.timeline.getPeriodCount();
        int windowCount = eventTime.timeline.getWindowCount();
        logd("timeline [" + getEventTimeString(eventTime) + ", periodCount=" + periodCount + ", windowCount=" + windowCount + ", reason=" + getTimelineChangeReasonString(r10));
        for (int r2 = 0; r2 < Math.min(periodCount, 3); r2++) {
            eventTime.timeline.getPeriod(r2, this.period);
            logd("  period [" + getTimeString(this.period.getDurationMs()) + "]");
        }
        if (periodCount > 3) {
            logd("  ...");
        }
        for (int r102 = 0; r102 < Math.min(windowCount, 3); r102++) {
            eventTime.timeline.getWindow(r102, this.window);
    ...
    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public void onMediaItemTransition(AnalyticsListener.EventTime eventTime, MediaItem mediaItem, int r4) {
        logd("mediaItem [" + getEventTimeString(eventTime) + ", reason=" + getMediaItemTransitionReasonString(r4) + "]");
    }
    ...
    public void onTracksChanged(AnalyticsListener.EventTime eventTime, Tracks tracks) {
        ...
        for (int r0 = 0; r0 < groups.size(); r0++) {
            Tracks.Group group = groups.get(r0);
            logd("  group [");
            for (int r4 = 0; r4 < group.length; r4++) {
                String trackStatusString = getTrackStatusString(group.isTrackSelected(r4));
                String formatSupportString = Util.getFormatSupportString(group.getTrackSupport(r4));
                logd("    " + trackStatusString + " Track:" + r4 + ", " + Format.toLogString(group.getTrackFormat(r4)) + ", supported=" + formatSupportString);
            }
            logd("  ]");
        }
        boolean z = false;
        for (int r1 = 0; !z && r1 < groups.size(); r1++) {
            Tracks.Group group2 = groups.get(r1);
            for (int r5 = 0; !z && r5 < group2.length; r5++) {
                if (group2.isTrackSelected(r5) && (metadata = group2.getTrackFormat(r5).metadata) != null && metadata.length() > 0) {
                    logd("  Metadata [");
    ...
    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public void onAudioUnderrun(AnalyticsListener.EventTime eventTime, int r3, long j, long j2) {
        loge(eventTime, "audioTrackUnderrun", r3 + ", " + j + ", " + j2, null);
    }
    ...
    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public void onAudioSessionIdChanged(AnalyticsListener.EventTime eventTime, int r3) {
        logd(eventTime, "audioSessionId", Integer.toString(r3));
    }
    ...
    public void onDroppedVideoFrames(AnalyticsListener.EventTime eventTime, int r2, long j) {
        logd(eventTime, "droppedFrames", Integer.toString(r2));
    }
    ...
    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public void onSurfaceSizeChanged(AnalyticsListener.EventTime eventTime, int r3, int r4) {
        logd(eventTime, "surfaceSize", r3 + ", " + r4);
    }
    ...
    @Override // com.google.android.exoplayer2.analytics.AnalyticsListener
    public void onDrmSessionAcquired(AnalyticsListener.EventTime eventTime, int r4) {
        logd(eventTime, "drmSessionAcquired", "state=" + r4);
    }
    ...
    protected void logd(String str) {
        Log.m1138d(this.tag, str);
    }

    protected void loge(String str) {
        Log.m1136e(this.tag, str);
    }
    ...
    private void printMetadata(Metadata metadata, String str) {
        for (int r0 = 0; r0 < metadata.length(); r0++) {
            logd(str + metadata.get(r0));
        }
    }
    ...
    private static String getTimeString(long j) {
        return j == C1856C.TIME_UNSET ? "?" : TIME_FORMAT.format(((float) j) / 1000.0f);
    }
}
