...
public final class PsExtractor implements Extractor {
    ...
    public void seek(long j, long j2) {
        boolean z = true;
        boolean z2 = this.timestampAdjuster.getTimestampOffsetUs() == C1856C.TIME_UNSET;
        if (!z2) {
            long firstSampleTimestampUs = this.timestampAdjuster.getFirstSampleTimestampUs();
            z2 = (firstSampleTimestampUs == C1856C.TIME_UNSET || firstSampleTimestampUs == 0 || firstSampleTimestampUs == j2) ? false : false;
        }
        if (z2) {
        ...
        for (int r3 = 0; r3 < this.psPayloadReaders.size(); r3++) {
            this.psPayloadReaders.valueAt(r3).seek();
        }
    }
    ...
    public int read(ExtractorInput extractorInput, PositionHolder positionHolder) throws IOException {
        ...
        int r6 = (length > (-1L) ? 1 : (length == (-1L) ? 0 : -1));
        if ((r6 != 0) && !this.durationReader.isDurationReadFinished()) {
        ...
        long peekPosition = r6 != 0 ? length - extractorInput.getPeekPosition() : -1L;
        if ((peekPosition == -1 || peekPosition >= 4) && extractorInput.peekFully(this.psPacketBuffer.getData(), 0, 4, true)) {
            ...
            if (readInt == PACK_START_CODE) {
                ...
                int r12 = readInt & 255;
                PesReader pesReader = this.psPayloadReaders.get(r12);
                if (!this.foundAllTracks) {
                    if (pesReader == null) {
                        ...
                        if (r12 == 189) {
                            ...
                            this.foundAudioTrack = true;
                            this.lastTrackPosition = extractorInput.getPosition();
                        } else if ((r12 & VIDEO_STREAM_MASK) == 224) {
                            elementaryStreamReader = new H262Reader();
                            this.foundVideoTrack = true;
                            this.lastTrackPosition = extractorInput.getPosition();
                        }
                        if (elementaryStreamReader != null) {
                            elementaryStreamReader.createTracks(this.output, new TsPayloadReader.TrackIdGenerator(r12, 256));
                            pesReader = new PesReader(elementaryStreamReader, this.timestampAdjuster);
                            this.psPayloadReaders.put(r12, pesReader);
                        }
                    }
