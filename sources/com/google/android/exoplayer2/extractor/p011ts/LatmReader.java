...
public final class LatmReader implements ElementaryStreamReader {
    ...
    @RequiresNonNull({"output"})
    private void parsePayloadMux(ParsableBitArray parsableBitArray, int r10) {
        int position = parsableBitArray.getPosition();
        if ((position & 7) == 0) {
            this.sampleDataBuffer.setPosition(position >> 3);
        } else {
            parsableBitArray.readBits(this.sampleDataBuffer.getData(), 0, r10 * 8);
            this.sampleDataBuffer.setPosition(0);
        }
        this.output.sampleData(this.sampleDataBuffer, r10);
        long j = this.timeUs;
        if (j != C1856C.TIME_UNSET) {
            this.output.sampleMetadata(j, 1, r10, 0, null);
            this.timeUs += this.sampleDurationUs;
        }
    }

    private void resetBufferForSize(int r2) {
        this.sampleDataBuffer.reset(r2);
