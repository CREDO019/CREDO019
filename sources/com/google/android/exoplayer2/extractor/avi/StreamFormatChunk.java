...
final class StreamFormatChunk implements AviChunk {
    ...
    private static String getMimeTypeFromCompression(int r0) {
        switch (r0) {
            case 808802372:
            case 877677894:
            case 1145656883:
    ...
    private static String getMimeTypeFromTag(int r1) {
        if (r1 != 1) {
            if (r1 != 85) {
                if (r1 != 255) {
                    if (r1 != 8192) {
    ...
    public static AviChunk parseFrom(int r1, ParsableByteArray parsableByteArray) {
        if (r1 == 2) {
        ...
        Log.m1132w(TAG, "Ignoring strf box for unsupported track type: " + Util.getTrackTypeString(r1));
        return null;
    }
    ...
    private static AviChunk parseBitmapInfoHeader(ParsableByteArray parsableByteArray) {
        ...
        if (mimeTypeFromCompression == null) {
            Log.m1132w(TAG, "Ignoring track with unsupported compression " + readLittleEndianInt3);
            return null;
        }
    ...
    private static AviChunk parseWaveFormatEx(ParsableByteArray parsableByteArray) {
        ...
        if (mimeTypeFromTag == null) {
            Log.m1132w(TAG, "Ignoring track with unsupported format tag " + readLittleEndianUnsignedShort);
            return null;
        }
        ...
        if (MimeTypes.AUDIO_AAC.equals(mimeTypeFromTag) && readLittleEndianUnsignedShort3 > 0) {
            builder.setInitializationData(ImmutableList.m408of(bArr));
        }
        return new StreamFormatChunk(builder.build());
    }
}
