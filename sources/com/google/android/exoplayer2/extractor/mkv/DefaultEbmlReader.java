...
final class DefaultEbmlReader implements EbmlReader {
    ...
    private long readInteger(ExtractorInput extractorInput, int r8) throws IOException {
        extractorInput.readFully(this.scratch, 0, r8);
        long j = 0;
        for (int r1 = 0; r1 < r8; r1++) {
            j = (j << 8) | (this.scratch[r1] & 255);
        }
        return j;
    }

    private double readFloat(ExtractorInput extractorInput, int r4) throws IOException {
        long readInteger = readInteger(extractorInput, r4);
        if (r4 == 4) {
    ...
    private static String readString(ExtractorInput extractorInput, int r3) throws IOException {
        if (r3 == 0) {
            return "";
        }
        byte[] bArr = new byte[r3];
        extractorInput.readFully(bArr, 0, r3);
        while (r3 > 0 && bArr[r3 - 1] == 0) {
            r3--;
        }
        return new String(bArr, 0, r3);
    }
    ...
    private static final class MasterElement {
        ...
        private MasterElement(int r1, long j) {
            this.elementId = r1;
