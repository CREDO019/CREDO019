...
public final class StarRating extends Rating {
    ...
    public StarRating(int r3) {
        Assertions.checkArgument(r3 > 0, "maxStars must be a positive integer");
        this.maxStars = r3;
        this.starRating = -1.0f;
    }

    public StarRating(int r5, float f) {
        boolean z = true;
        Assertions.checkArgument(r5 > 0, "maxStars must be a positive integer");
        Assertions.checkArgument((f < 0.0f || f > ((float) r5)) ? false : false, "starRating is out of range [0, maxStars]");
        this.maxStars = r5;
        this.starRating = f;
    }
    ...
    public static StarRating fromBundle(Bundle bundle) {
        ...
        int r0 = bundle.getInt(keyForField(1), 5);
        float f = bundle.getFloat(keyForField(2), -1.0f);
        if (f == -1.0f) {
            return new StarRating(r0);
        }
        return new StarRating(r0, f);
    }
