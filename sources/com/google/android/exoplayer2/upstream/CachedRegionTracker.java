...
public final class CachedRegionTracker implements Cache.Listener {
    ...
    public synchronized int getRegionEndTimeMs(long j) {
        ...
        if (floor != null && j <= floor.endOffset && floor.endOffsetIndex != -1) {
            int r8 = floor.endOffsetIndex;
            if (r8 == this.chunkIndex.length - 1) {
                if (floor.endOffset == this.chunkIndex.offsets[r8] + this.chunkIndex.sizes[r8]) {
                    return -2;
                }
            }
            return (int) ((this.chunkIndex.timesUs[r8] + ((this.chunkIndex.durationsUs[r8] * (floor.endOffset - this.chunkIndex.offsets[r8])) / this.chunkIndex.sizes[r8])) / 1000);
        }
        return -1;
    }
    ...
    public synchronized void onSpanRemoved(Cache cache, CacheSpan cacheSpan) {
        ...
        if (floor == null) {
            Log.m1136e(TAG, "Removed a span we were not aware of");
            return;
        }
    ...
    private void mergeSpan(CacheSpan cacheSpan) {
        ...
        if (regionsConnect(region, ceiling)) {
            ...
            int r0 = floor.endOffsetIndex;
            while (r0 < this.chunkIndex.length - 1) {
                int r2 = r0 + 1;
                if (this.chunkIndex.offsets[r2] > floor.endOffset) {
                    break;
                }
                r0 = r2;
            }
            floor.endOffsetIndex = r0;
        } else {
