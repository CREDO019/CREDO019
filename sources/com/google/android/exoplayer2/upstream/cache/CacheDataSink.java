...
public final class CacheDataSink implements DataSink {
    ...
    public static final class Factory implements DataSink.Factory {
        ...
        public Factory setBufferSize(int r1) {
            this.bufferSize = r1;
            return this;
        }
    ...
    public CacheDataSink(Cache cache, long j, int r9) {
        Assertions.checkState(j > 0 || j == -1, "fragmentSize must be positive or C.LENGTH_UNSET.");
        int r2 = (j > (-1L) ? 1 : (j == (-1L) ? 0 : -1));
        if (r2 != 0 && j < 2097152) {
            Log.m1132w(TAG, "fragmentSize is below the minimum recommended value of 2097152. This may cause poor cache performance.");
        }
        this.cache = (Cache) Assertions.checkNotNull(cache);
        this.fragmentSize = r2 == 0 ? Long.MAX_VALUE : j;
        this.bufferSize = r9;
    }
    ...
    @Override // com.google.android.exoplayer2.upstream.DataSink
    public void write(byte[] bArr, int r10, int r11) throws CacheDataSinkException {
        ...
        while (r1 < r11) {
            try {
                ...
                int min = (int) Math.min(r11 - r1, this.dataSpecFragmentSize - this.outputStreamBytesWritten);
                ((OutputStream) Util.castNonNull(this.outputStream)).write(bArr, r10 + r1, min);
                r1 += min;
                long j = min;
