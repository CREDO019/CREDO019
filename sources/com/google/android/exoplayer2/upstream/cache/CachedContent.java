...
public final class CachedContent {
    ...
    public CachedContent(int r2, String str) {
        this(r2, str, DefaultContentMetadata.EMPTY);
    }

    public CachedContent(int r1, String str, DefaultContentMetadata defaultContentMetadata) {
        this.f253id = r1;
        this.key = str;
    ...
    public boolean isFullyLocked(long j, long j2) {
        for (int r1 = 0; r1 < this.lockedRanges.size(); r1++) {
            if (this.lockedRanges.get(r1).contains(j, j2)) {
                return true;
            }
        }
        return false;
    ...
    public boolean lockRange(long j, long j2) {
        for (int r1 = 0; r1 < this.lockedRanges.size(); r1++) {
            if (this.lockedRanges.get(r1).intersects(j, j2)) {
                return false;
            }
        }
    ...
    public void unlockRange(long j) {
        for (int r0 = 0; r0 < this.lockedRanges.size(); r0++) {
            if (this.lockedRanges.get(r0).position == j) {
                this.lockedRanges.remove(r0);
                return;
            }
        }
    ...
    public SimpleCacheSpan setLastTouchTimestamp(SimpleCacheSpan simpleCacheSpan, long j, boolean z) {
        ...
        if (z) {
            File cacheFile = SimpleCacheSpan.getCacheFile((File) Assertions.checkNotNull(file.getParentFile()), this.f253id, simpleCacheSpan.position, j);
            if (file.renameTo(cacheFile)) {
                file = cacheFile;
            } else {
                Log.m1132w(TAG, "Failed to rename " + file + " to " + cacheFile);
            }
        }
    ...
    public int hashCode() {
        return (((this.f253id * 31) + this.key.hashCode()) * 31) + this.metadata.hashCode();
    }
    ...
    public boolean equals(Object obj) {
        ...
        return this.f253id == cachedContent.f253id && this.key.equals(cachedContent.key) && this.cachedSpans.equals(cachedContent.cachedSpans) && this.metadata.equals(cachedContent.metadata);
    }
