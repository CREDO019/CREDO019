...
public class JobInfoScheduler implements WorkScheduler {
    ...
    private boolean isJobServiceOn(JobScheduler jobScheduler, int r6, int r7) {
        for (JobInfo jobInfo : jobScheduler.getAllPendingJobs()) {
            int r2 = jobInfo.getExtras().getInt(ATTEMPT_NUMBER);
            if (jobInfo.getId() == r6) {
                return r2 >= r7;
            }
        }
    ...
    public void schedule(TransportContext transportContext, int r3) {
        schedule(transportContext, r3, false);
    }

    @Override // com.google.android.datatransport.runtime.scheduling.jobscheduling.WorkScheduler
    public void schedule(TransportContext transportContext, int r14, boolean z) {
        ...
        if (!z && isJobServiceOn(jobScheduler, jobId, r14)) {
            Logging.m1232d(LOG_TAG, "Upload for context %s is already scheduled. Returning...", transportContext);
            return;
        }
        long nextCallTime = this.eventStore.getNextCallTime(transportContext);
        JobInfo.Builder configureJob = this.config.configureJob(new JobInfo.Builder(jobId, componentName), transportContext.getPriority(), nextCallTime, r14);
        PersistableBundle persistableBundle = new PersistableBundle();
        persistableBundle.putInt(ATTEMPT_NUMBER, r14);
        ...
        Logging.m1230d(LOG_TAG, "Scheduling upload for context %s with jobId=%d in %dms(Backend next call timestamp %d). Attempt %d", transportContext, Integer.valueOf(jobId), Long.valueOf(this.config.getScheduleDelay(transportContext.getPriority(), nextCallTime, r14)), Long.valueOf(nextCallTime), Integer.valueOf(r14));
