...
public class AnimationsManager implements ViewHierarchyObserver {
    ...
    public void onViewUpdate(View view, Snapshot snapshot, Snapshot snapshot2) {
        ...
        if (viewState == ViewState.Appearing) {
            boolean z = true;
            for (int r1 = 0; r1 < Snapshot.targetKeysToTransform.size(); r1++) {
                if (((Number) currentMap.get(Snapshot.currentKeysToTransform.get(r1))).doubleValue() != ((Number) targetMap.get(Snapshot.targetKeysToTransform.get(r1))).doubleValue()) {
                    z = false;
                }
            }
            if (z) {
        ...
        this.mNativeMethodsHolder.startAnimationForTag(valueOf.intValue(), TtmlNode.TAG_LAYOUT, hashMap);
    }
    ...
    public void notifyAboutEnd(int r4, boolean z) {
        if (z) {
            return;
        }
        ViewState viewState = this.mStates.get(Integer.valueOf(r4));
        if (viewState == ViewState.Appearing) {
            this.mStates.put(Integer.valueOf(r4), ViewState.Layout);
        }
        if (viewState == ViewState.Disappearing) {
            this.mStates.put(Integer.valueOf(r4), ViewState.ToRemove);
            this.mToRemove.add(Integer.valueOf(r4));
            scheduleCleaning();
        }
    }
    ...
    public void printSubTree(View view, int r7) {
        if (r7 == 0) {
            Log.v("rea", "----------------------");
        }
        if (view == null) {
        ...
        while (true) {
            ...
            printSubTree(viewGroup.getChildAt(r2), r7 + 1);
            r2++;
        }
    }
    ...
    private void scheduleCleaning() {
        ...
        this.mContext.runOnUiQueueThread(new Runnable() { // from class: com.swmansion.reanimated.layoutReanimation.AnimationsManager$$ExternalSyntheticLambda0
            ...
            public final void run() {
                AnimationsManager.this.m230x3b4b62ba(weakReference);
            }
        });
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    /* renamed from: lambda$scheduleCleaning$0$com-swmansion-reanimated-layoutReanimation-AnimationsManager */
    public /* synthetic */ void m230x3b4b62ba(WeakReference weakReference) {
    ...
    private void findRoot(View view, HashSet<Integer> hashSet) {
        ...
        if (r1 != -1) {
            hashSet.add(Integer.valueOf(r1));
        }
    }
    ...
    private boolean dfs(View view, View view2, HashSet<Integer> hashSet) {
        ...
        if (hashSet.contains(Integer.valueOf(view2.getId())) || !this.mStates.containsKey(Integer.valueOf(view2.getId()))) {
            ...
            if ((view2 instanceof ViewGroup) && (this.mViewManager.get(Integer.valueOf(view2.getId())) instanceof ViewGroupManager)) {
                ...
                for (int r5 = 0; r5 < viewGroupManager.getChildCount(viewGroup2); r5++) {
                    arrayList.add(viewGroupManager.getChildAt(viewGroup2, r5));
                }
    ...
    public HashMap<String, Float> prepareDataForAnimationWorklet(HashMap<String, Object> hashMap, boolean z) {
        ...
        if (currentActivity != null) {
            ...
            int r5 = displayMetrics.heightPixels;
            hashMap2.put("windowWidth", Float.valueOf(PixelUtil.toDIPFromPixel(displayMetrics.widthPixels)));
            hashMap2.put("windowHeight", Float.valueOf(PixelUtil.toDIPFromPixel(r5)));
        } else {
    ...
    public void updateLayout(View view, ViewManager viewManager, int r4, int r5, float f, float f2, float f3, float f4) {
        ...
        if (r4 % 10 == 1) {
            ...
            throw new IllegalViewOperationException("Trying to use view with tag " + r4 + " as a parent, but its Manager doesn't implement IViewManagerWithChildren");
        }
        view.layout(round, round2, round3 + round, round4 + round2);
    }
