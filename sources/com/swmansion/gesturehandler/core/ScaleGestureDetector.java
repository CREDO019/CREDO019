...
public class ScaleGestureDetector {
    ...
    public ScaleGestureDetector(Context context, OnScaleGestureListener onScaleGestureListener, Handler handler) {
        ...
        int r2 = context.getApplicationInfo().targetSdkVersion;
        if (r2 > 18) {
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        int r4 = z5 ? pointerCount - 1 : pointerCount;
        if (inAnchoredScaleMode()) {
            ...
            float f4 = 0.0f;
            float f5 = 0.0f;
            for (int r11 = 0; r11 < pointerCount; r11++) {
                if (actionIndex != r11) {
                    f4 += motionEvent.getX(r11);
                    f5 += motionEvent.getY(r11);
                }
            }
            float f6 = r4;
            float f7 = f4 / f6;
        ...
        float f8 = 0.0f;
        for (int r14 = 0; r14 < pointerCount; r14++) {
            if (actionIndex != r14) {
                f3 += Math.abs(motionEvent.getX(r14) - f2);
                f8 += Math.abs(motionEvent.getY(r14) - f);
            }
        }
        float f9 = r4;
        ...
        int r42 = inAnchoredScaleMode() ? this.mSpanSlop : this.mMinSpan;
        if (!this.mInProgress && hypot >= r42 && (z6 || Math.abs(hypot - this.mInitialSpan) > this.mSpanSlop)) {
            this.mCurrSpanX = f10;
            this.mPrevSpanX = f10;
            this.mCurrSpanY = f11;
            this.mPrevSpanY = f11;
