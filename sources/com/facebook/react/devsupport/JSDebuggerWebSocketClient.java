...
public class JSDebuggerWebSocketClient extends WebSocketListener {
    ...
    private void sendMessage(int r2, String str) {
        WebSocket webSocket = this.mWebSocket;
        if (webSocket == null) {
            triggerRequestFailure(r2, new IllegalStateException("WebSocket connection no longer valid"));
            return;
        }
        try {
            webSocket.send(str);
        } catch (Exception e) {
            triggerRequestFailure(r2, e);
        }
    }

    private void triggerRequestFailure(int r3, Throwable th) {
        JSDebuggerCallback jSDebuggerCallback = this.mCallbacks.get(Integer.valueOf(r3));
        if (jSDebuggerCallback != null) {
            this.mCallbacks.remove(Integer.valueOf(r3));
            jSDebuggerCallback.onFailure(th);
        }
    }

    private void triggerRequestSuccess(int r3, String str) {
        JSDebuggerCallback jSDebuggerCallback = this.mCallbacks.get(Integer.valueOf(r3));
        if (jSDebuggerCallback != null) {
            this.mCallbacks.remove(Integer.valueOf(r3));
    ...
    private void abort(String str, Throwable th) {
        FLog.m1327e(TAG, "Error occurred, shutting down websocket connection: " + str, th);
        closeQuietly();
