...
public class DevServerHelper {
    ...
    public void openPackagerConnection(final String str, final PackagerCommandListener packagerCommandListener) {
        if (this.mPackagerClient != null) {
            FLog.m1288w(ReactConstants.TAG, "Packager connection already open, nooping.");
        } else {
    ...
    public void openInspectorConnection() {
        if (this.mInspectorPackagerConnection != null) {
            FLog.m1288w(ReactConstants.TAG, "Inspector connection already open, nooping.");
        } else {
        ...
        new AsyncTask<Void, String, Boolean>() { // from class: com.facebook.react.devsupport.DevServerHelper.5
            ...
            public boolean doSync() {
                try {
                    ...
                    new OkHttpClient().newCall(new Request.Builder().url(openUrlEndpoint).post(RequestBody.create(MediaType.parse("application/json"), new JSONObject().put(ImagesContract.URL, str).toString())).build()).execute();
                    return true;
                } catch (IOException | JSONException e) {
                    FLog.m1327e(ReactConstants.TAG, "Failed to open URL" + str, e);
                    return false;
                }
            }
            ...
            public void onPostExecute(Boolean bool) {
                ...
                RNLog.m1249w(reactContext, str2);
            }
        }.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, new Void[0]);
    }
        ...
        try {
            ...
            ((Call) Assertions.assertNotNull(this.mClient.newCall(new Request.Builder().url(createSymbolicateURL).post(RequestBody.create(MediaType.parse("application/json"), new JSONObject().put("stack", jSONArray).toString())).build()))).enqueue(new Callback() { // from class: com.facebook.react.devsupport.DevServerHelper.6
                ...
                public void onFailure(Call call, IOException iOException) {
                    FLog.m1288w(ReactConstants.TAG, "Got IOException when attempting symbolicate stack trace: " + iOException.getMessage());
                ...
                public void onResponse(Call call, C5015Response c5015Response) throws IOException {
                    try {
                        symbolicationListener.onSymbolicationComplete(Arrays.asList(StackTraceHelper.convertJsStackTrace(new JSONObject(c5015Response.body().string()).getJSONArray("stack"))));
            ...
            FLog.m1288w(ReactConstants.TAG, "Got JSONException when attempting symbolicate stack trace: " + e.getMessage());
        }
    }
    ...
    public void openStackFrameCall(StackFrame stackFrame) {
        ((Call) Assertions.assertNotNull(this.mClient.newCall(new Request.Builder().url(createOpenStackFrameURL(this.mSettings.getPackagerConnectionSettings().getDebugServerHost())).post(RequestBody.create(MediaType.parse("application/json"), stackFrame.toJSON().toString())).build()))).enqueue(new Callback() { // from class: com.facebook.react.devsupport.DevServerHelper.7
            ...
            @Override // okhttp3.Callback
            public void onFailure(Call call, IOException iOException) {
                FLog.m1288w(ReactConstants.TAG, "Got IOException when attempting to open stack frame: " + iOException.getMessage());
            }
        });
    }
    ...
    private String createBundleURL(String str, BundleType bundleType, String str2, boolean z, boolean z2) {
        ...
        objArr[6] = z ? "true" : Constants.CASEFIRST_FALSE;
        objArr[7] = z2 ? "true" : Constants.CASEFIRST_FALSE;
        objArr[8] = "";
    ...
    public void isPackagerRunning(PackagerStatusCallback packagerStatusCallback) {
        ...
        if (debugServerHost == null) {
            FLog.m1288w(ReactConstants.TAG, "No packager host configured.");
    ...
    public File downloadBundleResourceFromUrlSync(String str, File file) {
        Sink sink;
        try {
            C5015Response execute = this.mClient.newCall(new Request.Builder().url(createResourceURL(this.mSettings.getPackagerConnectionSettings().getDebugServerHost(), str)).build()).execute();
            ...
            FLog.m1326e(ReactConstants.TAG, "Failed to fetch resource synchronously - resourcePath: \"%s\", outputFile: \"%s\"", str, file.getAbsolutePath(), e);
            return null;
        }
    }
}
