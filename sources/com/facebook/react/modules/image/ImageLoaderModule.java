...
public class ImageLoaderModule extends NativeImageLoaderAndroidSpec implements LifecycleEventListener {
    ...
    public void prefetchImage(String str, double d, final Promise promise) {
        ...
        BaseDataSubscriber<Void> baseDataSubscriber = new BaseDataSubscriber<Void>() { // from class: com.facebook.react.modules.image.ImageLoaderModule.3
            ...
            protected void onNewResultImpl(DataSource<Void> dataSource) {
                if (dataSource.isFinished()) {
                    try {
                        try {
                            ImageLoaderModule.this.removeRequest(r3);
        ...
        registerRequest(r3, prefetchToDiskCache);
    ...
    private void registerRequest(int r3, DataSource<Void> dataSource) {
        synchronized (this.mEnqueuedRequestMonitor) {
            this.mEnqueuedRequests.put(r3, dataSource);
        }
    }

    /* JADX INFO: Access modifiers changed from: private */
    public DataSource<Void> removeRequest(int r4) {
        DataSource<Void> dataSource;
        synchronized (this.mEnqueuedRequestMonitor) {
            dataSource = this.mEnqueuedRequests.get(r4);
            this.mEnqueuedRequests.remove(r4);
        }
        return dataSource;
    }
    ...
    public void onHostDestroy() {
        synchronized (this.mEnqueuedRequestMonitor) {
            ...
            for (int r1 = 0; r1 < size; r1++) {
                DataSource<Void> valueAt = this.mEnqueuedRequests.valueAt(r1);
                if (valueAt != null) {
                    valueAt.close();
                }
            }
