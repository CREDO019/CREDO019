...
public class UIManagerHelper {
    ...
    public static UIManager getUIManagerForReactTag(ReactContext reactContext, int r1) {
        return getUIManager(reactContext, ViewUtil.getUIManagerType(r1));
    }

    public static UIManager getUIManager(ReactContext reactContext, int r2) {
        return getUIManager(reactContext, r2, true);
    }

    private static UIManager getUIManager(ReactContext reactContext, int r5, boolean z) {
        if (reactContext.isBridgeless()) {
            ...
            try {
                ...
                String str = TAG;
                ReactSoftExceptionLogger.logSoftException(str, new ReactNoCrashSoftException("Cannot get UIManager for UIManagerType: " + r5));
                return (UIManager) catalystInstance.getNativeModule(UIManagerModule.class);
            }
        }
    }

    public static EventDispatcher getEventDispatcherForReactTag(ReactContext reactContext, int r5) {
        EventDispatcher eventDispatcher = getEventDispatcher(reactContext, ViewUtil.getUIManagerType(r5));
        if (eventDispatcher == null) {
            String str = TAG;
            ReactSoftExceptionLogger.logSoftException(str, new IllegalStateException("Cannot get EventDispatcher for reactTag " + r5));
        }
        return eventDispatcher;
    }

    public static EventDispatcher getEventDispatcher(ReactContext reactContext, int r5) {
        if (reactContext.isBridgeless()) {
        ...
        UIManager uIManager = getUIManager(reactContext, r5, false);
        if (uIManager == null) {
            String str = TAG;
            ReactSoftExceptionLogger.logSoftException(str, new ReactNoCrashSoftException("Unable to find UIManager for UIManagerType " + r5));
            return null;
        }
        EventDispatcher eventDispatcher = (EventDispatcher) uIManager.getEventDispatcher();
        if (eventDispatcher == null) {
            String str2 = TAG;
            ReactSoftExceptionLogger.logSoftException(str2, new IllegalStateException("Cannot get EventDispatcher for UIManagerType " + r5));
        }
        return eventDispatcher;
    }
