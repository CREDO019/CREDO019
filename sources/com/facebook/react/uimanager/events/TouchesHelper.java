...
public class TouchesHelper {
    ...
    private static WritableMap[] createPointersArray(TouchEvent touchEvent) {
        ...
        for (int r4 = 0; r4 < motionEvent.getPointerCount(); r4++) {
            WritableMap createMap = Arguments.createMap();
            createMap.putDouble(PAGE_X_KEY, PixelUtil.toDIPFromPixel(motionEvent.getX(r4)));
            createMap.putDouble(PAGE_Y_KEY, PixelUtil.toDIPFromPixel(motionEvent.getY(r4)));
            createMap.putDouble(LOCATION_X_KEY, PixelUtil.toDIPFromPixel(motionEvent.getX(r4) - x));
            createMap.putDouble(LOCATION_Y_KEY, PixelUtil.toDIPFromPixel(motionEvent.getY(r4) - y));
            ...
            createMap.putDouble(POINTER_IDENTIFIER_KEY, motionEvent.getPointerId(r4));
            writableMapArr[r4] = createMap;
        }
    ...
    public static void sendTouchEvent(RCTModernEventEmitter rCTModernEventEmitter, TouchEvent touchEvent) {
        ...
        int r0 = C16961.$SwitchMap$com$facebook$react$uimanager$events$TouchEventType[touchEventType.ordinal()];
        if (r0 != 1) {
            if (r0 == 2) {
                ...
                for (int r3 = 0; r3 < createPointersArray.length; r3++) {
                    writableMapArr2[r3] = createPointersArray[r3].copy();
                }
            } else if (r0 != 4) {
