...
public class MountItemDispatcher {
    ...
    public boolean tryDispatchMountItems() {
        ...
        try {
            ...
            int r2 = this.mReDispatchCounter;
            if (r2 < 10 && dispatchMountItems) {
                if (r2 > 2) {
    ...
    private void executeOrEnqueue(MountItem mountItem) {
        if (this.mMountingManager.isWaitingForViewAttach(mountItem.getSurfaceId())) {
            if (FabricUIManager.ENABLE_FABRIC_LOGS) {
                FLog.m1326e(TAG, "executeOrEnqueue: Item execution delayed, surface %s is not ready yet", Integer.valueOf(mountItem.getSurfaceId()));
            }
