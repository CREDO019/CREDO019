...
class InterpolationAnimatedNode extends ValueAnimatedNode {
    ...
    private static double[] fromDoubleArray(ReadableArray readableArray) {
        ...
        for (int r2 = 0; r2 < size; r2++) {
            dArr[r2] = readableArray.getDouble(r2);
        }
        return dArr;
    }
    ...
    */
    ...
    static double interpolate(double d, double[] dArr, double[] dArr2, String str, String str2) {
        ...
        int r1 = findRangeIndex + 1;
        return interpolate(d, dArr[findRangeIndex], dArr[r1], dArr2[findRangeIndex], dArr2[r1], str, str2);
    }

    private static int findRangeIndex(double d, double[] dArr) {
        int r1 = 1;
        while (r1 < dArr.length - 1 && dArr[r1] < d) {
            r1++;
        }
        return r1 - 1;
    }
    ...
    public InterpolationAnimatedNode(ReadableMap readableMap) {
        ...
        if (z) {
            ...
            for (int r4 = 0; r4 < size; r4++) {
                Matcher matcher = fpPattern.matcher(array.getString(r4));
                ArrayList arrayList2 = new ArrayList();
                arrayList.add(arrayList2);
                while (matcher.find()) {
                    arrayList2.add(Double.valueOf(Double.parseDouble(matcher.group())));
                }
                this.mOutputRange[r4] = ((Double) arrayList2.get(0)).doubleValue();
            }
            int size2 = ((ArrayList) arrayList.get(0)).size();
            this.mNumVals = size2;
            this.mOutputs = new double[size2];
            for (int r0 = 0; r0 < this.mNumVals; r0++) {
                double[] dArr = new double[size];
                this.mOutputs[r0] = dArr;
                for (int r5 = 0; r5 < size; r5++) {
                    dArr[r5] = ((Double) ((ArrayList) arrayList.get(r5)).get(r0)).doubleValue();
                }
            }
        } else {
        ...
        if (this.mHasStringOutput) {
            if (this.mNumVals > 1) {
                ...
                while (this.mSOutputMatcher.find()) {
                    int r11 = r1 + 1;
                    double interpolate = interpolate(value, this.mInputRange, this.mOutputs[r1], this.mExtrapolateLeft, this.mExtrapolateRight);
                    if (this.mShouldRound) {
                        ...
                        int r3 = (int) interpolate;
                        this.mSOutputMatcher.appendReplacement(stringBuffer, ((double) r3) != interpolate ? Double.toString(interpolate) : Integer.toString(r3));
                    }
                    r1 = r11;
                }
