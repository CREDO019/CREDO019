...
public class ReactDelegate {
    ...
    public void onActivityResult(int r2, int r3, Intent intent, boolean z) {
        if (getReactNativeHost().hasInstance() && z) {
            getReactNativeHost().getReactInstanceManager().onActivityResult(this.mActivity, r2, r3, intent);
        }
    }
    ...
    public boolean shouldShowDevMenuOrReload(int r3, KeyEvent keyEvent) {
        if (getReactNativeHost().hasInstance() && getReactNativeHost().getUseDeveloperSupport()) {
            if (r3 == 82) {
                getReactNativeHost().getReactInstanceManager().showDevOptionsDialog();
                return true;
            } else if (((DoubleTapReloadRecognizer) Assertions.assertNotNull(this.mDoubleTapReloadRecognizer)).didDoubleTapR(r3, this.mActivity.getCurrentFocus())) {
