...
public class FadeDrawable extends ArrayDrawable {
    ...
    public void fadeUpToLayer(int index) {
        this.mTransitionState = 0;
        int r4 = index + 1;
        Arrays.fill(this.mIsLayerOn, 0, r4, true);
        Arrays.fill(this.mIsLayerOn, r4, this.mLayers.length, false);
        invalidateSelf();
    }
    ...
    public void finishTransitionImmediately() {
        this.mTransitionState = 2;
        for (int r1 = 0; r1 < this.mLayers.length; r1++) {
            this.mAlphas[r1] = this.mIsLayerOn[r1] ? 255 : 0;
        }
        invalidateSelf();
    }
    ...
    private boolean updateAlphas(float ratio) {
        boolean z = true;
        for (int r2 = 0; r2 < this.mLayers.length; r2++) {
            boolean[] zArr = this.mIsLayerOn;
            int r5 = zArr[r2] ? 1 : -1;
            int[] r6 = this.mAlphas;
            r6[r2] = (int) (this.mStartAlphas[r2] + (r5 * 255 * ratio));
            if (r6[r2] < 0) {
                r6[r2] = 0;
            }
            if (r6[r2] > 255) {
    ...
    */
    ...
    private void maybeOnFadeStarted() {
        int r0;
        if (!this.mIsFadingActualImage && (r0 = this.mActualImageLayer) >= 0) {
            boolean[] zArr = this.mIsLayerOn;
            if (r0 < zArr.length && zArr[r0]) {
