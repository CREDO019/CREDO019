...
public class DecodeProducer implements Producer<CloseableReference<CloseableImage>> {
    ...
    private class LocalImagesProgressiveDecoder extends ProgressiveDecoder {
        ...
        protected QualityInfo getQualityInfo() {
            return ImmutableQualityInfo.m1265of(0, false, false);
        }
    }
    ...
    private class NetworkImagesProgressiveDecoder extends ProgressiveDecoder {
        ...
        protected synchronized boolean updateDecodeJob(@Nullable EncodedImage encodedImage, int status) {
            ...
            if ((isNotLast(status) || statusHasFlag(status, 8)) && !statusHasFlag(status, 4) && EncodedImage.isValid(encodedImage) && encodedImage.getImageFormat() == DefaultImageFormats.JPEG) {
                ...
                int r1 = this.mLastScheduledScanNumber;
                if (bestScanNumber <= r1) {
                    return false;
                }
                if (bestScanNumber < this.mProgressiveJpegConfig.getNextScanNumberToDecode(r1) && !this.mProgressiveJpegParser.isEndMarkerRead()) {
                    return false;
                }
