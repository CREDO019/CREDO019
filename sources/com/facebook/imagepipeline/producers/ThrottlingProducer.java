...
public class ThrottlingProducer<T> implements Producer<T> {
    ...
    static /* synthetic */ int access$210(ThrottlingProducer throttlingProducer) {
        int r0 = throttlingProducer.mNumCurrentRequests;
        throttlingProducer.mNumCurrentRequests = r0 - 1;
        return r0;
    }
    ...
    public void produceResults(final Consumer<T> consumer, final ProducerContext producerContext) {
        ...
        synchronized (this) {
            int r0 = this.mNumCurrentRequests;
            z = true;
            if (r0 >= this.mMaxSimultaneousRequests) {
                this.mPendingRequests.add(Pair.create(consumer, producerContext));
            } else {
                this.mNumCurrentRequests = r0 + 1;
                z = false;
            }
        }
        if (z) {
