...
public class PartialDiskCacheProducer implements Producer<EncodedImage> {
    ...
    static Map<String, String> getExtraMap(final ProducerListener2 listener, final ProducerContext producerContext, final boolean valueFound, final int sizeInBytes) {
        if (listener.requiresExtraMap(producerContext, PRODUCER_NAME)) {
            if (valueFound) {
                return ImmutableMap.m1355of("cached_value_found", String.valueOf(valueFound), "encodedImageSize", String.valueOf(sizeInBytes));
            }
            return ImmutableMap.m1356of("cached_value_found", String.valueOf(valueFound));
        }
        return null;
    }
    ...
    public static class PartialDiskCacheConsumer extends DelegatingConsumer<EncodedImage, EncodedImage> {
            ...
            if (this.mPartialEncodedImageFromCache != null && newResult != null && newResult.getBytesRange() != null) {
                try {
                    try {
                        ...
                        FLog.m1327e(PartialDiskCacheProducer.PRODUCER_NAME, "Error while merging image data", e);
        ...
        private PooledByteBufferOutputStream merge(EncodedImage initialData, EncodedImage remainingData) throws IOException {
            int r0 = ((BytesRange) Preconditions.checkNotNull(remainingData.getBytesRange())).from;
            PooledByteBufferOutputStream newOutputStream = this.mPooledByteBufferFactory.newOutputStream(remainingData.getSize() + r0);
            copy(initialData.getInputStreamOrThrow(), newOutputStream, r0);
        ...
        private void copy(InputStream from, OutputStream to, int length) throws IOException {
            ...
            while (r2 > 0) {
                try {
                    int read = from.read(bArr, 0, Math.min(16384, r2));
                    if (read < 0) {
                        break;
                    } else if (read > 0) {
            ...
            if (r2 > 0) {
                throw new IOException(String.format(null, "Failed to read %d bytes - finished %d short", Integer.valueOf(length), Integer.valueOf(r2)));
            }
        }
        ...
        private void sendFinalResultToConsumer(PooledByteBufferOutputStream pooledOutputStream) {
            ...
            Throwable th;
            CloseableReference m1274of = CloseableReference.m1274of(pooledOutputStream.toByteBuffer());
            try {
