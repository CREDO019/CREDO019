...
public class PriorityNetworkFetcher<FETCH_STATE extends FetchState> implements NetworkFetcher<PriorityFetchState<FETCH_STATE>> {
    ...
    public void fetch(final PriorityFetchState<FETCH_STATE> fetchState, final NetworkFetcher.Callback callback) {
        ...
        synchronized (this.mLock) {
            if (this.mCurrentlyFetching.contains(fetchState)) {
                String str = TAG;
                FLog.m1328e(str, "fetch state was enqueued twice: " + fetchState);
                return;
            }
            boolean z = fetchState.getContext().getPriority() == Priority.HIGH;
            FLog.m1298v(TAG, "enqueue: %s %s", z ? "HI-PRI" : "LOW-PRI", fetchState.getUri());
    ...
    public void removeFromQueue(PriorityFetchState<FETCH_STATE> fetchState, String reasonForLogging) {
        synchronized (this.mLock) {
            FLog.m1298v(TAG, "remove: %s %s", reasonForLogging, fetchState.getUri());
    ...
    public void requeue(PriorityFetchState<FETCH_STATE> fetchState) {
        synchronized (this.mLock) {
            FLog.m1299v(TAG, "requeue: %s", fetchState.getUri());
            boolean z = true;
    ...
    private void dequeueIfAvailableSlots() {
        if (this.isRunning) {
            synchronized (this.mLock) {
                ...
                FLog.m1296v(TAG, "fetching: %s (concurrent: %s hi-pri queue: %s low-pri queue: %s)", pollFirst.getUri(), Integer.valueOf(size), Integer.valueOf(this.mHiPriQueue.size()), Integer.valueOf(this.mLowPriQueue.size()));
    ...
    public void changePriority(PriorityFetchState<FETCH_STATE> fetchState, boolean isNewHiPri) {
        synchronized (this.mLock) {
            ...
            FLog.m1298v(TAG, "change-pri: %s %s", isNewHiPri ? "HIPRI" : "LOWPRI", fetchState.getUri());
            fetchState.priorityChangedCount++;
