...
public class BufferedDiskCache {
    ...
    private Task<Boolean> containsAsync(final CacheKey key) {
        try {
            ...
            FLog.m1289w(TAG, e, "Failed to schedule disk-cache read for %s", key.getUriString());
    ...
    public Task<Void> probe(final CacheKey key) {
        ...
        try {
            ...
            FLog.m1289w(TAG, e, "Failed to schedule disk-cache probe for %s", key.getUriString());
            return Task.forError(e);
        }
    }
    ...
    public boolean checkInStagingAreaAndFileCache(final CacheKey key) {
        ...
        if (encodedImage != null) {
            encodedImage.close();
            FLog.m1307v(TAG, "Found image for %s in staging area", key.getUriString());
            this.mImageCacheStatsTracker.onStagingAreaHit(key);
            return true;
        }
        FLog.m1307v(TAG, "Did not find image for %s in staging area", key.getUriString());
            ...
            return Task.call(new Callable<EncodedImage>() { // from class: com.facebook.imagepipeline.cache.BufferedDiskCache.3
                ...
                public EncodedImage call() throws Exception {
                    ...
                    try {
                        ...
                        if (encodedImage != null) {
                            FLog.m1307v(BufferedDiskCache.TAG, "Found image for %s in staging area", key.getUriString());
                            BufferedDiskCache.this.mImageCacheStatsTracker.onStagingAreaHit(key);
                        } else {
                            FLog.m1307v(BufferedDiskCache.TAG, "Did not find image for %s in staging area", key.getUriString());
                            ...
                            try {
                                ...
                                CloseableReference m1274of = CloseableReference.m1274of(readFromDiskCache);
                                try {
                                    encodedImage = new EncodedImage(m1274of);
                                } finally {
                                    CloseableReference.closeSafely(m1274of);
                                }
                            } catch (Exception unused) {
                                return null;
                            }
                        ...
                        if (Thread.interrupted()) {
                            FLog.m1308v(BufferedDiskCache.TAG, "Host thread was interrupted, decreasing reference count");
                            if (encodedImage != null) {
                                encodedImage.close();
                            }
    ...
    public void put(final CacheKey key, EncodedImage encodedImage) {
        try {
            ...
            try {
                ...
                FLog.m1289w(TAG, e, "Failed to schedule disk-cache write for %s", key.getUriString());
    ...
    public Task<Void> remove(final CacheKey key) {
        ...
        try {
            ...
            FLog.m1289w(TAG, e, "Failed to schedule disk-cache remove for %s", key.getUriString());
            return Task.forError(e);
        }
    }
    ...
    public Task<Void> clearAll() {
        ...
        try {
            ...
            FLog.m1289w(TAG, e, "Failed to schedule disk-cache clear", new Object[0]);
            return Task.forError(e);
        }
    }
    ...
    public PooledByteBuffer readFromDiskCache(final CacheKey key) throws IOException {
        try {
            Class<?> cls = TAG;
            FLog.m1307v(cls, "Disk cache read for %s", key.getUriString());
            BinaryResource resource = this.mFileCache.getResource(key);
            if (resource == null) {
                FLog.m1307v(cls, "Disk cache miss for %s", key.getUriString());
                this.mImageCacheStatsTracker.onDiskCacheMiss(key);
                return null;
            }
            FLog.m1307v(cls, "Found entry in disk cache for %s", key.getUriString());
            ...
            openStream.close();
            FLog.m1307v(cls, "Successful read from disk cache for %s", key.getUriString());
            return newByteBuffer;
        } catch (IOException e) {
            FLog.m1289w(TAG, e, "Exception reading from cache for %s", key.getUriString());
    ...
    public void writeToDiskCache(final CacheKey key, final EncodedImage encodedImage) {
        Class<?> cls = TAG;
        FLog.m1307v(cls, "About to write to disk-cache for key %s", key.getUriString());
        try {
            ...
            FLog.m1307v(cls, "Successful disk-cache write for key %s", key.getUriString());
        } catch (IOException e) {
            FLog.m1289w(TAG, e, "Failed to write to disk-cache for key %s", key.getUriString());
        }
    }
}
