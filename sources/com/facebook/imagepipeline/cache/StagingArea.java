...
public class StagingArea {
    ...
    public void clearAll() {
        ...
        for (int r1 = 0; r1 < arrayList.size(); r1++) {
            EncodedImage encodedImage = (EncodedImage) arrayList.get(r1);
        ...
        if (encodedImage != null) {
            synchronized (encodedImage) {
                if (!EncodedImage.isValid(encodedImage)) {
                    this.mMap.remove(key);
                    FLog.m1290w(TAG, "Found closed reference %d for key %s (%d)", Integer.valueOf(System.identityHashCode(encodedImage)), key.getUriString(), Integer.valueOf(System.identityHashCode(key)));
                    return null;
                }
    ...
    private synchronized void logStats() {
        FLog.m1307v(TAG, "Count = %d", Integer.valueOf(this.mMap.size()));
    }
}
