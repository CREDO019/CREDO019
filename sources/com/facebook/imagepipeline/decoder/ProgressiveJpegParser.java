...
public class ProgressiveJpegParser {
    ...
    private boolean doParseMoreData(final InputStream inputStream) {
        int read;
        int r0 = this.mBestScanNumber;
        while (this.mParserState != 6 && (read = inputStream.read()) != -1) {
            try {
                int r5 = this.mBytesParsed + 1;
                this.mBytesParsed = r5;
                if (this.mEndMarkerRead) {
                    this.mParserState = 6;
                    this.mEndMarkerRead = false;
                    return false;
                }
                int r6 = this.mParserState;
                if (r6 != 0) {
                    if (r6 != 1) {
                        if (r6 != 2) {
                            if (r6 != 3) {
                                if (r6 == 4) {
                                    ...
                                    int r52 = ((this.mLastByteRead << 8) + read) - 2;
                                    StreamUtil.skip(inputStream, r52);
                                    this.mBytesParsed += r52;
                                    this.mParserState = 2;
                                } else {
        ...
        return (this.mParserState == 6 || this.mBestScanNumber == r0) ? false : true;
    }

    private void newScanOrImageEndFound(int offset) {
        int r0 = this.mNextFullScanNumber;
        if (r0 > 0) {
            this.mBestScanEndOffset = offset;
        }
        this.mNextFullScanNumber = r0 + 1;
        this.mBestScanNumber = r0;
    }
