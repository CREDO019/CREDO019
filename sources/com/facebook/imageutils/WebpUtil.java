...
public class WebpUtil {
    ...
    private static boolean compare(byte[] what, String with) {
        ...
        for (int r0 = 0; r0 < what.length; r0++) {
            if (with.charAt(r0) != what[r0]) {
                return false;
            }
        }
        return true;
    ...
    private static int getInt(InputStream is) throws IOException {
        return ((((byte) is.read()) << Ascii.CAN) & ViewCompat.MEASURED_STATE_MASK) | ((((byte) is.read()) << 16) & 16711680) | ((((byte) is.read()) << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (((byte) is.read()) & 255);
    }
