...
public class ByteBufferGifDecoder implements ResourceDecoder<ByteBuffer, GifDrawable> {
    ...
    public GifDrawableResource decode(ByteBuffer byteBuffer, int r9, int r10, Options options) {
        ...
        try {
            return decode(byteBuffer, r9, r10, obtain, options);
        } finally {
    ...
    private GifDrawableResource decode(ByteBuffer byteBuffer, int r18, int r19, GifHeaderParser gifHeaderParser, Options options) {
        ...
        try {
            ...
            if (parseHeader.getNumFrames() > 0 && parseHeader.getStatus() == 0) {
                ...
                GifDecoder build = this.gifDecoderFactory.build(this.provider, parseHeader, byteBuffer, getSampleSize(parseHeader, r18, r19));
                ...
                GifDrawableResource gifDrawableResource = new GifDrawableResource(new GifDrawable(this.context, build, UnitTransformation.get(), r18, r19, nextFrame));
    ...
    private static int getSampleSize(GifHeader gifHeader, int r5, int r6) {
        int min = Math.min(gifHeader.getHeight() / r6, gifHeader.getWidth() / r5);
        int max = Math.max(1, min == 0 ? 0 : Integer.highestOneBit(min));
        if (Log.isLoggable(TAG, 2) && max > 1) {
            Log.v(TAG, "Downsampling GIF, sampleSize: " + max + ", target dimens: [" + r5 + "x" + r6 + "], actual dimens: [" + gifHeader.getWidth() + "x" + gifHeader.getHeight() + "]");
        }
        return max;
    }
    ...
    public static class GifDecoderFactory {
        ...
        GifDecoder build(GifDecoder.BitmapProvider bitmapProvider, GifHeader gifHeader, ByteBuffer byteBuffer, int r5) {
            return new StandardGifDecoder(bitmapProvider, gifHeader, byteBuffer, r5);
        }
    }
