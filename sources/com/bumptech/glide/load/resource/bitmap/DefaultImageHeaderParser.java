...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    private int parseExifSegment(Reader reader, byte[] bArr, int r6) throws IOException {
        int read = reader.read(bArr, r6);
        if (read != r6) {
            if (Log.isLoggable(TAG, 3)) {
                Log.d(TAG, "Unable to read exif segment data, length: " + r6 + ", actually read: " + read);
            }
            return -1;
        } else if (hasJpegExifPreamble(bArr, r6)) {
    ...
    private boolean hasJpegExifPreamble(byte[] bArr, int r6) {
        boolean z = bArr != null && r6 > JPEG_EXIF_SEGMENT_PREAMBLE_BYTES.length;
        if (z) {
            int r1 = 0;
            while (true) {
    ...
    private static int parseExifSegment(RandomAccessReader randomAccessReader) {
        ...
        for (int r2 = 0; r2 < int162; r2++) {
            int calcTagOffset = calcTagOffset(int32, r2);
                ...
                if (int164 < 1 || int164 > 12) {
                    ...
                    if (int322 < 0) {
                        ...
                        if (r8 > 4) {
                            ...
                            int r5 = calcTagOffset + 8;
                            if (r5 < 0 || r5 > randomAccessReader.length()) {
                                if (Log.isLoggable(TAG, 3)) {
                                    Log.d(TAG, "Illegal tagValueOffset=" + r5 + " tagType=" + ((int) int163));
                                }
                            } else if (r8 < 0 || r8 + r5 > randomAccessReader.length()) {
                                if (Log.isLoggable(TAG, 3)) {
                                    Log.d(TAG, "Illegal number of bytes for TI tag data tagType=" + ((int) int163));
                                }
                            } else {
                                return randomAccessReader.getInt16(r5);
                            }
                        }
                    }
                }
    ...
    public static final class RandomAccessReader {
        ...
        private boolean isAvailable(int r2, int r3) {
            return this.data.remaining() - r2 >= r3;
        }
    }
    ...
    private static final class ByteBufferReader implements Reader {
        ...
        @Override // com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser.Reader
        public int read(byte[] bArr, int r4) {
            int min = Math.min(r4, this.byteBuffer.remaining());
            if (min == 0) {
                return -1;
            }
    ...
    private static final class StreamReader implements Reader {
        ...
        /* renamed from: is */
        private final InputStream f111is;

        StreamReader(InputStream inputStream) {
            this.f111is = inputStream;
        }

        @Override // com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser.Reader
        public int getUInt16() throws IOException {
            return ((this.f111is.read() << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (this.f111is.read() & 255);
        }

        @Override // com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser.Reader
        public short getUInt8() throws IOException {
            return (short) (this.f111is.read() & 255);
        }
        ...
        public long skip(long j) throws IOException {
            ...
            long j2 = j;
            while (j2 > 0) {
                long skip = this.f111is.skip(j2);
                if (skip <= 0) {
                    if (this.f111is.read() == -1) {
                        break;
                    }
                    skip = 1;
                }
        ...
        @Override // com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser.Reader
        public int read(byte[] bArr, int r5) throws IOException {
            int r0 = r5;
            while (r0 > 0) {
                int read = this.f111is.read(bArr, r5 - r0, r0);
                if (read == -1) {
                    break;
                }
                r0 -= read;
