...
public final class LruArrayPool implements ArrayPool {
    ...
    public synchronized <T> void put(T t) {
        ...
        if (isSmallEnoughForReuse(elementSizeInBytes)) {
            ...
            sizesForAdapter.put(valueOf, Integer.valueOf(r3));
    ...
    public synchronized <T> T get(int r3, Class<T> cls) {
        Key key;
        Integer ceilingKey = getSizesForAdapter(cls).ceilingKey(Integer.valueOf(r3));
        if (mayFillRequest(r3, ceilingKey)) {
    ...
    private boolean isSmallEnoughForReuse(int r2) {
        return r2 <= this.maxSize / 2;
    }

    private boolean mayFillRequest(int r2, Integer num) {
        return num != null && (isNoMoreThanHalfFull() || num.intValue() <= r2 * 8);
    }
    ...
    private void decrementArrayOfSize(int r4, Class<?> cls) {
        NavigableMap<Integer, Integer> sizesForAdapter = getSizesForAdapter(cls);
        Integer num = (Integer) sizesForAdapter.get(Integer.valueOf(r4));
        if (num == null) {
            throw new NullPointerException("Tried to decrement empty size, size: " + r4 + ", this: " + this);
        } else if (num.intValue() == 1) {
            sizesForAdapter.remove(Integer.valueOf(r4));
        } else {
            sizesForAdapter.put(Integer.valueOf(r4), Integer.valueOf(num.intValue() - 1));
        }
    }
