...
public abstract class BaseRequestOptions<T extends BaseRequestOptions<T>> implements Cloneable {
    ...
    public T sizeMultiplier(float f) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().sizeMultiplier(f);
        }
        if (f < 0.0f || f > 1.0f) {
    ...
    public T useUnlimitedSourceGeneratorsPool(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().useUnlimitedSourceGeneratorsPool(z);
        }
    ...
    public T useAnimationPool(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().useAnimationPool(z);
        }
    ...
    public T onlyRetrieveFromCache(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().onlyRetrieveFromCache(z);
        }
    ...
    public T diskCacheStrategy(DiskCacheStrategy diskCacheStrategy) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().diskCacheStrategy(diskCacheStrategy);
        }
    ...
    public T priority(Priority priority) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().priority(priority);
        }
    ...
    public T placeholder(Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().placeholder(drawable);
        }
    ...
    public T placeholder(int r2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().placeholder(r2);
        }
    ...
    public T fallback(Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().fallback(drawable);
        }
    ...
    public T fallback(int r2) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().fallback(r2);
        }
        this.fallbackId = r2;
    ...
    public T error(Drawable drawable) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().error(drawable);
        }
    ...
    public T theme(Resources.Theme theme) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().theme(theme);
        }
        this.theme = theme;
        this.fields |= 32768;
        return selfOrThrowIfLocked();
    ...
    public T skipMemoryCache(boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().skipMemoryCache(true);
        }
        this.isCacheable = !z;
        this.fields |= 256;
        return selfOrThrowIfLocked();
    ...
    public T override(int r2, int r3) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().override(r2, r3);
        }
    ...
    public T override(int r1) {
        return override(r1, r1);
    }

    public T signature(Key key) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().signature(key);
        }
    ...
    public <Y> T set(Option<Y> option, Y y) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().set(option, y);
        }
    ...
    public T decode(Class<?> cls) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().decode(cls);
        }
    ...
    public T encodeQuality(int r2) {
        return set(BitmapEncoder.COMPRESSION_QUALITY, Integer.valueOf(r2));
    }
    ...
    public T timeout(int r2) {
        return set(HttpGlideUrlLoader.TIMEOUT, Integer.valueOf(r2));
    }
    ...
    final T optionalTransform(DownsampleStrategy downsampleStrategy, Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().optionalTransform(downsampleStrategy, transformation);
        }
    ...
    final T transform(DownsampleStrategy downsampleStrategy, Transformation<Bitmap> transformation) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().transform(downsampleStrategy, transformation);
        }
        downsample(downsampleStrategy);
    ...
    T transform(Transformation<Bitmap> transformation, boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().transform(transformation, z);
        }
    ...
    <Y> T transform(Class<Y> cls, Transformation<Y> transformation, boolean z) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().transform(cls, transformation, z);
        }
        Preconditions.checkNotNull(cls);
        Preconditions.checkNotNull(transformation);
        this.transformations.put(cls, transformation);
        this.isTransformationAllowed = true;
        int r2 = this.fields | 2048 | 65536;
        this.fields = r2;
        this.isScaleOnlyOrNoTransform = false;
        if (z) {
            this.fields = r2 | 131072;
    ...
    public T dontTransform() {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().dontTransform();
        }
    ...
    public T apply(BaseRequestOptions<?> baseRequestOptions) {
        if (this.isAutoCloneEnabled) {
            return (T) mo1484clone().apply(baseRequestOptions);
        }
