...
public class ReactSliderManager extends SimpleViewManager<ReactSlider> {
    private static final SeekBar.OnSeekBarChangeListener ON_CHANGE_LISTENER = new SeekBar.OnSeekBarChangeListener() { // from class: com.reactnativecommunity.slider.ReactSliderManager.1
        ...
        public void onProgressChanged(SeekBar seekBar, int r6, boolean z) {
            ReactSlider reactSlider = (ReactSlider) seekBar;
            if (r6 < reactSlider.getLowerLimit()) {
                r6 = reactSlider.getLowerLimit();
                seekBar.setProgress(r6);
            } else if (r6 > reactSlider.getUpperLimit()) {
                r6 = reactSlider.getUpperLimit();
                seekBar.setProgress(r6);
            }
            ReactContext reactContext = (ReactContext) seekBar.getContext();
            if (z) {
                ((UIManagerModule) reactContext.getNativeModule(UIManagerModule.class)).getEventDispatcher().dispatchEvent(new ReactSliderEvent(seekBar.getId(), reactSlider.toRealProgress(r6), true));
            }
        }
