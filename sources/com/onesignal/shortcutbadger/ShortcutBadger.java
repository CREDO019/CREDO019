...
public final class ShortcutBadger {
    ...
    public static boolean applyCount(Context context, int r2) {
        try {
            applyCountOrThrow(context, r2);
            return true;
    ...
    public static void applyCountOrThrow(Context context, int r3) throws ShortcutBadgeException {
        if (sShortcutBadger == null && !initBadger(context)) {
            throw new ShortcutBadgeException("No default launcher available");
        }
        try {
            sShortcutBadger.executeBadge(context, sComponentName, r3);
        } catch (Exception e) {
            ...
            synchronized (sCounterSupportedLock) {
                if (sIsBadgeCounterSupported == null) {
                    ...
                    for (int r3 = 0; r3 < 3; r3++) {
                        try {
                            Log.i(LOG_TAG, "Checking if platform supports badge counters, attempt " + String.format("%d/%d.", Integer.valueOf(r3 + 1), 3));
                        } catch (Exception e) {
                            str = e.getMessage();
                        }
    ...
    public static void applyNotification(Context context, Notification notification, int r7) {
        if (Build.MANUFACTURER.equalsIgnoreCase("Xiaomi")) {
            try {
                Object obj = notification.getClass().getDeclaredField("extraNotification").get(notification);
                obj.getClass().getDeclaredMethod("setMessageCount", Integer.TYPE).invoke(obj, Integer.valueOf(r7));
            } catch (Exception e) {
                if (Log.isLoggable(LOG_TAG, 3)) {
                    Log.d(LOG_TAG, "Unable to execute badge", e);
                }
