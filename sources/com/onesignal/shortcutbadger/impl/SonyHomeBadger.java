...
public class SonyHomeBadger implements Badger {
    ...
    public void executeBadge(Context context, ComponentName componentName, int r4) throws ShortcutBadgeException {
        if (sonyBadgeContentProviderExists(context)) {
            executeBadgeByContentProvider(context, componentName, r4);
        } else {
            executeBadgeByBroadcast(context, componentName, r4);
        }
    }
    ...
    private static void executeBadgeByBroadcast(Context context, ComponentName componentName, int r5) {
        Intent intent = new Intent(INTENT_ACTION);
        intent.putExtra(INTENT_EXTRA_PACKAGE_NAME, componentName.getPackageName());
        intent.putExtra(INTENT_EXTRA_ACTIVITY_NAME, componentName.getClassName());
        intent.putExtra(INTENT_EXTRA_MESSAGE, String.valueOf(r5));
        intent.putExtra(INTENT_EXTRA_SHOW_MESSAGE, r5 > 0);
        context.sendBroadcast(intent);
    }

    private void executeBadgeByContentProvider(Context context, ComponentName componentName, int r4) {
        if (r4 < 0) {
            return;
        }
        ContentValues createContentValues = createContentValues(r4, componentName);
    ...
    private ContentValues createContentValues(int r3, ComponentName componentName) {
        ContentValues contentValues = new ContentValues();
        contentValues.put(PROVIDER_COLUMNS_BADGE_COUNT, Integer.valueOf(r3));
