...
public class BiometricFragment extends Fragment {
    ...
    public void onActivityResult(int r1, int r2, Intent intent) {
        ...
        if (r1 == 1) {
            ...
            handleConfirmCredentialResult(r2);
        }
    }
    ...
    public void authenticate(BiometricPrompt.PromptInfo promptInfo, BiometricPrompt.CryptoObject cryptoObject) {
        ...
        if (isManagingDeviceCredentialButton()) {
            this.mViewModel.setNegativeButtonTextOverride(getString(C0214R.string.confirm_device_credential_password));
        } else {
    ...
    void authenticateWithBiometricPrompt(android.hardware.biometrics.BiometricPrompt biometricPrompt, Context context) {
        ...
        try {
            ...
            sendErrorAndDismiss(1, context != null ? context.getString(C0214R.string.default_error_msg) : "");
        }
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public void cancelAuthentication(int r2) {
        if (r2 == 3 || !this.mViewModel.isIgnoringCancel()) {
            if (isUsingFingerprintDialog()) {
                this.mViewModel.setCanceledFrom(r2);
                if (r2 == 1) {
    ...
    void onAuthenticationError(final int r4, final CharSequence charSequence) {
        if (!ErrorUtils.isKnownError(r4)) {
            r4 = 8;
        }
        Context context = getContext();
        if (Build.VERSION.SDK_INT >= 21 && Build.VERSION.SDK_INT < 29 && ErrorUtils.isLockoutError(r4) && context != null && KeyguardUtils.isDeviceSecuredWithCredential(context) && AuthenticatorUtils.isDeviceCredentialAllowed(this.mViewModel.getAllowedAuthenticators())) {
            launchConfirmCredentialActivity();
        } else if (!isUsingFingerprintDialog()) {
            if (charSequence == null) {
                charSequence = getString(C0214R.string.default_error_msg) + " " + r4;
            }
            sendErrorAndDismiss(r4, charSequence);
        } else {
            if (charSequence == null) {
                charSequence = ErrorUtils.getFingerprintErrorString(getContext(), r4);
            }
            if (r4 == 5) {
                int canceledFrom = this.mViewModel.getCanceledFrom();
                if (canceledFrom == 0 || canceledFrom == 3) {
                    sendErrorToClient(r4, charSequence);
                }
                dismiss();
                return;
            }
            if (this.mViewModel.isFingerprintDialogDismissedInstantly()) {
                ...
                this.mHandler.postDelayed(new Runnable() { // from class: androidx.biometric.BiometricFragment.8
                    @Override // java.lang.Runnable
                    public void run() {
                        BiometricFragment.this.sendErrorAndDismiss(r4, charSequence);
                    }
    ...
    void onAuthenticationFailed() {
        if (isUsingFingerprintDialog()) {
            showFingerprintErrorMessage(getString(C0214R.string.fingerprint_not_recognized));
        }
        sendFailureToClient();
    }
    ...
    void onCancelButtonPressed() {
        ...
        if (negativeButtonText == null) {
            negativeButtonText = getString(C0214R.string.default_error_msg);
        }
    ...
    private void launchConfirmCredentialActivity() {
        ...
        if (keyguardManager == null) {
            sendErrorAndDismiss(12, getString(C0214R.string.generic_error_no_keyguard));
            return;
        }
        CharSequence title = this.mViewModel.getTitle();
        CharSequence subtitle = this.mViewModel.getSubtitle();
        ...
        if (createConfirmDeviceCredentialIntent == null) {
            sendErrorAndDismiss(14, getString(C0214R.string.generic_error_no_device_credential));
            return;
        }
    ...
    private void handleConfirmCredentialResult(int r3) {
        if (r3 == -1) {
            sendSuccessAndDismiss(new BiometricPrompt.AuthenticationResult(null, 1));
        } else {
            sendErrorAndDismiss(10, getString(C0214R.string.generic_error_user_canceled));
        }
    }

    private void showFingerprintErrorMessage(CharSequence charSequence) {
        if (charSequence == null) {
            charSequence = getString(C0214R.string.default_error_msg);
        }
    ...
    void sendErrorAndDismiss(int r1, CharSequence charSequence) {
        sendErrorToClient(r1, charSequence);
        dismiss();
    }
    ...
    private void sendErrorToClient(final int r3, final CharSequence charSequence) {
        if (this.mViewModel.isConfirmingDeviceCredential()) {
            ...
            this.mViewModel.getClientExecutor().execute(new Runnable() { // from class: androidx.biometric.BiometricFragment.10
                @Override // java.lang.Runnable
                public void run() {
                    BiometricFragment.this.mViewModel.getClientCallback().onAuthenticationError(r3, charSequence);
                }
            });
        }
    }
    ...
    public static class Api30Impl {
        ...
        static void setAllowedAuthenticators(BiometricPrompt.Builder builder, int r1) {
            builder.setAllowedAuthenticators(r1);
        }
    }
