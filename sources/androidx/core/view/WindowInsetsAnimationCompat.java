...
public final class WindowInsetsAnimationCompat {
    ...
    public WindowInsetsAnimationCompat(int r3, Interpolator interpolator, long j) {
        if (Build.VERSION.SDK_INT >= 30) {
            this.mImpl = new Impl30(r3, interpolator, j);
    ...
    public static class Impl {
        ...
        Impl(int r1, Interpolator interpolator, long j) {
            this.mTypeMask = r1;
    ...
    public static class Impl21 extends Impl {
        Impl21(int r1, Interpolator interpolator, long j) {
            super(r1, interpolator, j);
        }

        static void setCallback(View view, Callback callback) {
            Object tag = view.getTag(C0264R.C0267id.tag_on_apply_window_listener);
            if (callback == null) {
                view.setTag(C0264R.C0267id.tag_window_insets_animation_callback, null);
                if (tag == null) {
            ...
            view.setTag(C0264R.C0267id.tag_window_insets_animation_callback, createProxyListener);
            if (tag == null) {
        ...
        static BoundsCompat computeAnimationBounds(WindowInsetsCompat windowInsetsCompat, WindowInsetsCompat windowInsetsCompat2, int r6) {
            Insets insets = windowInsetsCompat.getInsets(r6);
            Insets insets2 = windowInsetsCompat2.getInsets(r6);
            return new BoundsCompat(Insets.m1421of(Math.min(insets.left, insets2.left), Math.min(insets.top, insets2.top), Math.min(insets.right, insets2.right), Math.min(insets.bottom, insets2.bottom)), Insets.m1421of(Math.max(insets.left, insets2.left), Math.max(insets.top, insets2.top), Math.max(insets.right, insets2.right), Math.max(insets.bottom, insets2.bottom)));
        }
        ...
        static int buildAnimationMask(WindowInsetsCompat windowInsetsCompat, WindowInsetsCompat windowInsetsCompat2) {
            ...
            for (int r0 = 1; r0 <= 256; r0 <<= 1) {
                if (!windowInsetsCompat.getInsets(r0).equals(windowInsetsCompat2.getInsets(r0))) {
                    r1 |= r0;
                }
            }
            return r1;
        ...
        static WindowInsetsCompat interpolateInsets(WindowInsetsCompat windowInsetsCompat, WindowInsetsCompat windowInsetsCompat2, float f, int r15) {
            WindowInsetsCompat.Builder builder = new WindowInsetsCompat.Builder(windowInsetsCompat);
            for (int r1 = 1; r1 <= 256; r1 <<= 1) {
                if ((r15 & r1) == 0) {
                    builder.setInsets(r1, windowInsetsCompat.getInsets(r1));
                } else {
                    Insets insets = windowInsetsCompat.getInsets(r1);
                    Insets insets2 = windowInsetsCompat2.getInsets(r1);
                    float f2 = 1.0f - f;
                    builder.setInsets(r1, WindowInsetsCompat.insetInsets(insets, (int) (((insets.left - insets2.left) * f2) + 0.5d), (int) (((insets.top - insets2.top) * f2) + 0.5d), (int) (((insets.right - insets2.right) * f2) + 0.5d), (int) (((insets.bottom - insets2.bottom) * f2) + 0.5d)));
                }
            }
        ...
        static WindowInsets forwardToViewIfNeeded(View view, WindowInsets windowInsets) {
            return view.getTag(C0264R.C0267id.tag_on_apply_window_listener) != null ? windowInsets : view.onApplyWindowInsets(windowInsets);
        }
        ...
        static Callback getCallback(View view) {
            Object tag = view.getTag(C0264R.C0267id.tag_window_insets_animation_callback);
    ...
    public static class Impl30 extends Impl {
        ...
        Impl30(int r2, Interpolator interpolator, long j) {
            this(new WindowInsetsAnimation(r2, interpolator, j));
        }
