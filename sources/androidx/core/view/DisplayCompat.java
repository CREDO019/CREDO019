...
public final class DisplayCompat {
    ...
    public static class Api23Impl {
        ...
        public static ModeCompat[] getSupportedModes(Context context, Display display) {
            ...
            Point currentDisplaySizeFromWorkarounds = DisplayCompat.getCurrentDisplaySizeFromWorkarounds(context, display);
            if (currentDisplaySizeFromWorkarounds == null || physicalSizeEquals(mode, currentDisplaySizeFromWorkarounds)) {
                for (int r7 = 0; r7 < supportedModes.length; r7++) {
                    modeCompatArr[r7] = new ModeCompat(supportedModes[r7], physicalSizeEquals(supportedModes[r7], mode));
                }
            } else {
                for (int r3 = 0; r3 < supportedModes.length; r3++) {
                    if (physicalSizeEquals(supportedModes[r3], mode)) {
                        modeCompat = new ModeCompat(supportedModes[r3], currentDisplaySizeFromWorkarounds);
                    } else {
                        modeCompat = new ModeCompat(supportedModes[r3], false);
                    }
                    modeCompatArr[r3] = modeCompat;
                }
            }
        ...
        static boolean isCurrentModeTheLargestMode(Display display) {
            Display.Mode mode = display.getMode();
            Display.Mode[] supportedModes = display.getSupportedModes();
            for (int r2 = 0; r2 < supportedModes.length; r2++) {
                if (mode.getPhysicalHeight() < supportedModes[r2].getPhysicalHeight() || mode.getPhysicalWidth() < supportedModes[r2].getPhysicalWidth()) {
                    return false;
                }
            }
            return true;
