...
public class ViewPager extends ViewGroup {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(this, new OnApplyWindowInsetsListener() { // from class: androidx.viewpager.widget.ViewPager.4
            ...
            public WindowInsetsCompat onApplyWindowInsets(View view, WindowInsetsCompat windowInsetsCompat) {
                ...
                for (int r0 = 0; r0 < childCount; r0++) {
                    WindowInsetsCompat dispatchApplyWindowInsets = ViewCompat.dispatchApplyWindowInsets(ViewPager.this.getChildAt(r0), onApplyWindowInsets);
    ...
    void setCurrentItemInternal(int r2, boolean z, boolean z2) {
        setCurrentItemInternal(r2, z, z2, 0);
    }

    void setCurrentItemInternal(int r5, boolean z, boolean z2, int r8) {
        PagerAdapter pagerAdapter = this.mAdapter;
        if (pagerAdapter == null || pagerAdapter.getCount() <= 0) {
            setScrollingCacheEnabled(false);
        } else if (!z2 && this.mCurItem == r5 && this.mItems.size() != 0) {
            setScrollingCacheEnabled(false);
        } else {
            if (r5 < 0) {
                r5 = 0;
    ...
    public void setPageTransformer(boolean z, PageTransformer pageTransformer, int r7) {
    ...
    void smoothScrollTo(int r10, int r11, int r12) {
        ...
        float f = clientWidth;
        float f2 = r112;
        float distanceInfluenceForSnapDuration = f2 + (distanceInfluenceForSnapDuration(Math.min(1.0f, (Math.abs(r6) * 1.0f) / f)) * f2);
        int abs2 = Math.abs(r12);
        if (abs2 > 0) {
            abs = Math.round(Math.abs(distanceInfluenceForSnapDuration / abs2) * 1000.0f) * 4;
        } else {
            abs = (int) (((Math.abs(r6) / ((f * this.mAdapter.getPageWidth(this.mCurItem)) + this.mPageMargin)) + 1.0f) * 100.0f);
        }
    ...
    private void calculatePageOffsets(ItemInfo itemInfo, int r12, ItemInfo itemInfo2) {
            ...
            if (r3 < itemInfo.position) {
                ...
                while (r32 <= itemInfo.position && r13 < this.mItems.size()) {
                    ...
                    while (r32 < itemInfo4.position) {
                        f2 += this.mAdapter.getPageWidth(r32) + f;
                        r32++;
                    }
                    itemInfo4.offset = f2;
                ...
                while (true) {
                    ...
                    while (r3 > itemInfo3.position) {
                        f3 -= this.mAdapter.getPageWidth(r3) + f;
                        r3--;
                    }
        ...
        while (r5 >= 0) {
            ...
            while (r4 > itemInfo7.position) {
                f4 -= this.mAdapter.getPageWidth(r4) + f;
                r4--;
            }
        ...
        while (r122 < size2) {
            ...
            while (r11 < itemInfo8.position) {
                f5 += this.mAdapter.getPageWidth(r11) + f;
                r11++;
            }
    ...
    */
    ...
    private void recomputeScrollPosition(int r2, int r3, int r4, int r5) {
        ...
        int min = (int) ((infoForPosition != null ? Math.min(infoForPosition.offset, this.mLastOffset) : 0.0f) * ((r2 - getPaddingLeft()) - getPaddingRight()));
        if (min != getScrollX()) {
            completeScroll(false);
    ...
    */
    ...
    public boolean onInterceptTouchEvent(MotionEvent motionEvent) {
        ...
        if (action == 0) {
            ...
            if (r0 != -1) {
                ...
                int r14 = (f > 0.0f ? 1 : (f == 0.0f ? 0 : -1));
                if (r14 != 0 && !isGutterDrag(this.mLastMotionX, f) && canScroll(this, false, (int) f, (int) x2, (int) y2)) {
                ...
                if (abs > r02 && abs * 0.5f > abs2) {
                    ...
                    this.mLastMotionX = r14 > 0 ? f2 + f3 : f2 - f3;
                    this.mLastMotionY = y2;
                    setScrollingCacheEnabled(true);
                } else if (abs2 > r02) {
                    this.mIsUnableToDrag = true;
    ...
    private int determineTargetPage(int r2, float f, int r4, int r5) {
        ...
        if (this.mItems.size() > 0) {
            ArrayList<ItemInfo> arrayList = this.mItems;
            return Math.max(this.mItems.get(0).position, Math.min(r2, arrayList.get(arrayList.size() - 1).position));
        }
        return r2;
    }
    ...
    public class MyAccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public boolean performAccessibilityAction(View view, int r3, Bundle bundle) {
            ...
            if (r3 == 4096) {
                ...
                return false;
            } else if (r3 == 8192 && ViewPager.this.canScrollHorizontally(-1)) {
