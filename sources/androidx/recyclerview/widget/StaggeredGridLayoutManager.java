...
public class StaggeredGridLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    */
    ...
    public void setMeasuredDimension(Rect rect, int r6, int r7) {
        ...
        if (this.mOrientation == 1) {
            ...
            chooseSize = chooseSize(r6, (this.mSizePerSpan * this.mSpanCount) + paddingLeft, getMinimumWidth());
        } else {
            chooseSize = chooseSize(r6, rect.width() + paddingLeft, getMinimumWidth());
            chooseSize2 = chooseSize(r7, (this.mSizePerSpan * this.mSpanCount) + paddingTop, getMinimumHeight());
        }
    ...
    */
    ...
    private void measureChildWithDecorationsAndMargin(View view, int r6, int r7, boolean z) {
        ...
        int updateSpecWithExtra = updateSpecWithExtra(r6, layoutParams.leftMargin + this.mTmpRect.left, layoutParams.rightMargin + this.mTmpRect.right);
        int updateSpecWithExtra2 = updateSpecWithExtra(r7, layoutParams.topMargin + this.mTmpRect.top, layoutParams.bottomMargin + this.mTmpRect.bottom);
        if (z) {
    ...
    */
    ...
    private void setLayoutStateDirection(int r5) {
        ...
        this.mLayoutState.mItemDirection = this.mShouldReverseLayout != (r5 == -1) ? -1 : 1;
    }
    ...
    */
    ...
    private int fill(RecyclerView.Recycler recycler, LayoutState layoutState, RecyclerView.State state) {
        ...
        while (layoutState.hasMore(state) && (this.mLayoutState.mInfinite || !this.mRemainingSpans.isEmpty())) {
            ...
            if (layoutState.mLayoutDirection == 1) {
                int maxEnd2 = layoutParams.mFullSpan ? getMaxEnd(r12) : span3.getEndLine(r12);
                ...
                r5 = decoratedMeasurement3;
                decoratedMeasurement = maxEnd2;
            } else {
                int minStart = layoutParams.mFullSpan ? getMinStart(r12) : span3.getStartLine(r12);
            ...
            if (isLayoutRTL() && this.mOrientation == 1) {
                ...
                r3 = endAfterPadding - this.mSecondaryOrientation.getDecoratedMeasurement(next);
            } else {
                ...
                r3 = startAfterPadding2;
                decoratedMeasurement2 = this.mSecondaryOrientation.getDecoratedMeasurement(next) + startAfterPadding2;
            }
            if (this.mOrientation == 1) {
                layoutDecoratedWithMargins(next, r3, decoratedMeasurement, decoratedMeasurement2, r5);
            } else {
                layoutDecoratedWithMargins(next, decoratedMeasurement, r3, r5, decoratedMeasurement2);
            }
            if (layoutParams.mFullSpan) {
                updateAllRemainingSpans(this.mLayoutState.mLayoutDirection, r11);
            } else {
                updateRemainingSpans(span3, this.mLayoutState.mLayoutDirection, r11);
            }
    ...
    public void collectAdjacentPrefetchPositions(int r5, int r6, RecyclerView.State state, RecyclerView.LayoutManager.LayoutPrefetchRegistry layoutPrefetchRegistry) {
        ...
        for (int r62 = 0; r62 < this.mSpanCount; r62++) {
            if (this.mLayoutState.mItemDirection == -1) {
                ...
                r2 = this.mSpans[r62].getStartLine(this.mLayoutState.mStartLine);
            } else {
                endLine = this.mSpans[r62].getEndLine(this.mLayoutState.mEndLine);
                r2 = this.mLayoutState.mEndLine;
            }
            int r1 = endLine - r2;
            if (r1 >= 0) {
    ...
    void prepareLayoutStateForDelta(int r5, RecyclerView.State state) {
        int firstChildPosition;
        int r2;
        if (r5 > 0) {
            firstChildPosition = getLastChildPosition();
        ...
        this.mLayoutState.mAvailable = Math.abs(r5);
    }
    ...
    public View onFocusSearchFailed(View view, int r10, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        resolveShouldLayoutReverse();
        int convertFocusDirectionToLayoutDirection = convertFocusDirectionToLayoutDirection(r10);
        ...
        if (z || (focusableViewAfter = span.getFocusableViewAfter(firstChildPosition, convertFocusDirectionToLayoutDirection)) == null || focusableViewAfter == findContainingItemView) {
            if (preferLastSpan(convertFocusDirectionToLayoutDirection)) {
                for (int r11 = this.mSpanCount - 1; r11 >= 0; r11--) {
                    View focusableViewAfter2 = this.mSpans[r11].getFocusableViewAfter(firstChildPosition, convertFocusDirectionToLayoutDirection);
                ...
                for (int r112 = 0; r112 < this.mSpanCount; r112++) {
                    View focusableViewAfter3 = this.mSpans[r112].getFocusableViewAfter(firstChildPosition, convertFocusDirectionToLayoutDirection);
            ...
            if (preferLastSpan(convertFocusDirectionToLayoutDirection)) {
                for (int r102 = this.mSpanCount - 1; r102 >= 0; r102--) {
                    if (r102 != span.mIndex) {
                        if (z2) {
                            findLastPartiallyVisibleItemPosition2 = this.mSpans[r102].findFirstPartiallyVisibleItemPosition();
                        } else {
                            findLastPartiallyVisibleItemPosition2 = this.mSpans[r102].findLastPartiallyVisibleItemPosition();
                        }
                ...
                for (int r6 = 0; r6 < this.mSpanCount; r6++) {
                    if (z2) {
                        findLastPartiallyVisibleItemPosition = this.mSpans[r6].findFirstPartiallyVisibleItemPosition();
                    } else {
                        findLastPartiallyVisibleItemPosition = this.mSpans[r6].findLastPartiallyVisibleItemPosition();
                    }
    ...
    public static class LazySpanLookup {
        ...
        int invalidateAfter(int r4) {
            ...
            if (r0 != null && r4 < r0.length) {
                int invalidateFullSpansAfter = invalidateFullSpansAfter(r4);
                ...
                Arrays.fill(this.mData, r4, min, -1);
                return min;
            }
            return -1;
        }
        ...
        void setSpan(int r2, Span span) {
            ensureSize(r2);
            this.mData[r2] = span.mIndex;
        }
        ...
        private int invalidateFullSpansAfter(int r5) {
            ...
            while (true) {
                if (r2 >= size) {
                    r2 = -1;
                    break;
                } else if (this.mFullSpanItems.get(r2).mPosition >= r5) {
                    break;
                } else {
                    r2++;
                }
            ...
            if (r2 != -1) {
                ...
                return this.mFullSpanItems.get(r2).mPosition;
            }
            return -1;
        }
