...
public class WebSettingsCompat {
    ...
    public static void setDisabledActionModeMenuItems(WebSettings webSettings, int r3) {
        WebViewFeatureInternal webViewFeatureInternal = WebViewFeatureInternal.DISABLED_ACTION_MODE_MENU_ITEMS;
        if (webViewFeatureInternal.isSupportedByFramework()) {
            webSettings.setDisabledActionModeMenuItems(r3);
        } else if (webViewFeatureInternal.isSupportedByWebView()) {
            getAdapter(webSettings).setDisabledActionModeMenuItems(r3);
        } else {
            throw WebViewFeatureInternal.getUnsupportedOperationException();
        }
    }
    ...
    public static void setForceDark(WebSettings webSettings, int r3) {
        WebViewFeatureInternal webViewFeatureInternal = WebViewFeatureInternal.FORCE_DARK;
        if (webViewFeatureInternal.isSupportedByFramework()) {
            webSettings.setForceDark(r3);
        } else if (webViewFeatureInternal.isSupportedByWebView()) {
            getAdapter(webSettings).setForceDark(r3);
        } else {
            throw WebViewFeatureInternal.getUnsupportedOperationException();
        }
    }
    ...
    public static void setForceDarkStrategy(WebSettings webSettings, int r2) {
        if (WebViewFeatureInternal.FORCE_DARK_STRATEGY.isSupportedByWebView()) {
            getAdapter(webSettings).setForceDarkStrategy(r2);
            return;
        }
        throw WebViewFeatureInternal.getUnsupportedOperationException();
    }
