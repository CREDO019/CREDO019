...
public class SpeaktoitRecognitionServiceImpl extends AIService implements VoiceActivityDetector.SpeechEventsListener, MediaPlayer.OnCompletionListener, MediaPlayer.OnErrorListener {
    ...
    public SpeaktoitRecognitionServiceImpl(Context context, AIConfiguration aIConfiguration) {
        ...
        this.vad = new VoiceActivityDetector(16000);
    ...
    public class RecorderStream extends InputStream {
        ...
        public int read(byte[] bArr, int r6, int r7) throws IOException {
            int read = this.audioRecord.read(bArr, r6, r7);
            if (read > 0) {
        ...
        private void normalize(byte[] bArr, int r11) {
            int r0 = 4800 - this.offset;
            if (r11 >= r0) {
                ByteBuffer order = ByteBuffer.wrap(bArr, r0, r11 - r0).order(ByteOrder.LITTLE_ENDIAN);
                ShortBuffer asShortBuffer = order.asShortBuffer();
                for (int r3 = 0; r3 < asShortBuffer.limit(); r3++) {
                    short s = asShortBuffer.get(r3);
                ...
                for (int r2 = 0; r2 < asShortBuffer.limit(); r2++) {
                    order.putShort((short) ((asShortBuffer.get(r2) - this.alignment) * f2));
                }
            }
            this.offset += Math.min(r11, r0);
        }
    }
    ...
    public boolean onError(MediaPlayer mediaPlayer, int r2, int r3) {
        if (this.isRecording) {
            startRecording(this.extras);
            return false;
        }
